...
| 最大阻塞 | `DNS / TLS / 正式域名` 外部资产未落地，直接阻断 `G1 / G8` |
| 剩余收尾 | `G1 / G8` 外部硬阻塞解除与正式窗口日志 |
...
1. 解除 `DNS / TLS / 正式域名` 外部硬阻塞
2. 在解阻后补齐 `G8` 正式窗口 `server dry-run / apply / rollback` 日志
3. 在 [2026-07-19-v72-resign-bundle.md](file:///Users/yaoyunzhong/Desktop/shenjiying/shenjiying88/docs/knowledge/acceptance/2026-07-19-v72-resign-bundle.md) 上固化 `G1 / G8` 最新结论
4. 维持 `主计划 / 任务状态 / 复签检查表 / 复签总包` 四者口径一致
...
3. `G1 / G8` 仅剩外部资产解阻与正式窗口日志，不再有结构性缺口
