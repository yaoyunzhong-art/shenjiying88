...
| tenant-llm | PRD-014 | `apps/api/src/modules/tenant-llm/llm-config.service.ts` | `open-platform-ringbeam.test.ts` 中 AC-49-01/05/06/07/08 + `llm-config.tenant-service.test.ts` 32 例 + `llm-config.controller.test.ts` 29 例 | 多租户隔离、审批、统计、权限拒绝已有主链验证 |
...
1. `PRD-015` 目前仍停留在 PRD / 页面契约层，尚未补前台实现与监控验收。
2. P-49 当前以 service/controller 级圈梁为主，还缺更高层的 E2E 或 API smoke 审计。
