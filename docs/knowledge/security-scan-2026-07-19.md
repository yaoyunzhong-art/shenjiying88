# 🔐 安全扫描报告

> 扫描时间: 2026-07-19 00:01:31 CST
> 项目: shenjiying88 (V17)
> 扫描模式: full
> 脚本: scripts/security-scan.sh

---

## 📊 汇总

| 检查项 | 结果 | 数量 |
|--------|:----:|:----:|
| 硬编码密码 | 🟢 | 0 |
| 硬编码Token | 🔴 | 1 |
| 依赖危急漏洞 | 🟢 | 0 |
| 依赖高危漏洞 | 🟢 | 0 |
| **总体风险** | **🔴 中危** | **退出码: 2** |

---

## 1️⃣ 密钥泄漏检查

> 扫描范围: apps/api/src/ — 排除 test/spec/mock/example

| 文件位置 | 内容(脱敏) | 状态 |
|----------|-----------|:----:|
无发现 🟢

---

## 2️⃣ Token硬编码检查

> 扫描范围: apps/ — 排除 test/spec

| 文件位置 | 内容(脱敏) | 状态 |
|----------|-----------|:----:|
  | `apps/api/src/modules/tenant/tenant-isolation.service.ts:69` | `apps/api/src/modules/tenant/tenant-isolation.service.ts:69:        `Tenant misma` | ⚠️  |\n
---

## 3️⃣ 依赖审计

> 命令: pnpm audit --audit-level=high

✅ 无高/危急漏洞

---

## 🚦 闸门判定

| 条件 | 状态 | 说明 |
|------|:----:|------|
| 硬编码密码 > 0 | 🟢 | 🚫 硬阻断 — 必须修复后才能合并 |
| 硬编码Token > 0 | 🔴 | 🚫 硬阻断 — 必须修复后才能合并 |
| 依赖危急漏洞 > 0 | 🟢 | 🚫 硬阻断 — 必须修复后才能合并 |
| 依赖高危漏洞 > 0 | 🟢 | ⚠️ 建议修复 |
| **出口** | **🔴 中危 (退出码 2)** | |

> 🐜 [V17: security-gates] · 安全门硬阻断pipeline

