...
| finance 持久化主链 | 🟡 推进中 | 79% | PRD-007 已签发，任务表 `T168-finance` 状态失真 | 2026-07-21 已把 `FinanceService` 的 `ledger/account/settlement` 补成 Prisma write-through / resolved read-through，`FinanceController` 对账户/结算也开始优先走持久化链；本轮继续给 `FinanceReportService` 补了 `createReportResolved / regenerateReportResolved` 与报表聚合 resolved 读链，`FinanceReportController` 也开始优先走 resolved 主链；`finance-report.service.test.ts`、`finance-report-data-aggregation.test.ts` 已补 resolved 护栏，`finance.e2e.test.ts` 的 `/finance/reports` 也已改为复用真实 `FinanceReportController` 主链；本轮再把 `invoice` 改成 Prisma 优先写透 + resolved 读写链，`FinanceController` 与 `finance.e2e.test.ts` 的发票路由也开始优先走持久化主链 | P-54 验收只覆盖 checkout revenue 可见性，不等于 finance 全域持久化完成；当前已补静态诊断与 controller/e2e 接线，但真实 DB runtime 证据仍缺 | 继续补真实 DB 级回归证据，并把 report/export 与剩余 finance runtime 验收跑通 |
