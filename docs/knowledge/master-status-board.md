...
| finance 持久化主链 | 🟡 推进中 | 67% | PRD-007 已签发，任务表 `T168-finance` 状态失真 | 2026-07-21 已把 `FinanceService` 的 `ledger/account/settlement` 补成 Prisma write-through / resolved read-through，`FinanceController` 对账户/结算也开始优先走持久化链；同步兼容内存态 fallback 仍保留给历史测试 | P-54 验收只覆盖 checkout revenue 可见性，不等于 finance 全域持久化完成 | 继续把 report / e2e / 真实 DB 证据也切到 resolved 主链，并评估 invoice 是否需要同样持久化治理 |
| transactions / finance / sdk contract 唯一真源 | 🟡 推进中 | 86% | P-54 与共享收口零散分布 | SDK 已新增 `BusinessFinanceLedgerRecord`、`BusinessRevenueSummary`、`BusinessFinanceAccountRecord`、`BusinessFinanceSettlementRecord`，且 `finance.contract.ts` 已把 ledger/account/settlement/revenue contract 收口为 SDK 类型别名，`createBusinessClient().finance` 也补齐了账户/结算入口 | 缺单独 contract 收口验收，`invoice / daily revenue` 仍未进入 SDK 真源 | 继续让 finance Web / App 消费端改吃 SDK 真源，并评估是否把 `invoice / daily` 也纳入 SDK contract |
