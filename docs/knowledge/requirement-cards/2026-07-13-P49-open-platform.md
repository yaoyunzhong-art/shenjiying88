...
## 执行证据
- 圈梁测试: `apps/api/src/modules/openapi/open-platform-ringbeam.test.ts` (`14 tests`)
- HTTP smoke: `apps/api/src/modules/openapi/open-platform.e2e.test.ts` (`7 tests`)
- 租户服务测试: `apps/api/src/modules/tenant-llm/llm-config.tenant-service.test.ts` (`32 tests`)
- 租户控制器测试: `apps/api/src/modules/tenant-llm/llm-config.controller.test.ts` (`29 tests`)
- 租户 HTTP e2e: `apps/api/src/modules/tenant-llm/tenant-llm.e2e.test.ts` (`27 tests`)
