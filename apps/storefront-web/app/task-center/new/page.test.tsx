/**
 * apps/storefront-web/app/task-center/new/page.tsx — L1 冒烟测试
 * 角色视角: 👔店长 / 🏪全体门店员工
 * 覆盖: 正例·反例·边界
 */
...
describe('TaskCenterNew — 正例', () => {
  ...
  test('has mock data', () => { assert.ok(SRC.includes('MOCK') || SRC.includes('mock') || SRC.includes('TASK')); });
});

describe('TaskCenterNew — 反例', () => {
...
describe('TaskCenterNew — 边界', () => {
  test('has length check', () => { assert.ok(SRC.includes('.length')); });
  test('has conditional rendering', () => { assert.ok(SRC.includes('?')); });
  test('has task type options', () => { assert.ok(SRC.includes('TASK_TYPE') || SRC.includes('OPTIONS')); });
});
