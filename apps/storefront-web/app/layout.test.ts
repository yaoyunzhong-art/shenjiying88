...
test('正例: metadata contains title and description', async () => {
  const { metadata } = await import('./layout');
  assert.ok((metadata as Record<string, unknown>).title);
  assert.ok((metadata as Record<string, unknown>).description);
});
...
test('边界: layout default export has correct name', async () => {
  const mod = await import('./layout');
  const name = mod.default?.name || mod.default?.displayName || 'unknown';
  assert.ok(name.length > 0, 'component should have a name');
});

test('边界: metadata should not contain templating syntax', async () => {
  const { metadata } = await import('./layout');
  const metaStr = JSON.stringify(metadata);
  assert.ok(!metaStr.includes('{{'), 'title should not contain template syntax');
  assert.ok(!metaStr.includes('}}'), 'title should not contain template syntax');
});

test('正例: metadata viewport is configured', async () => {
  const { metadata } = await import('./layout');
  assert.ok((metadata as Record<string, unknown>).viewport !== undefined || 
            JSON.stringify(metadata).includes('viewport'));
});
