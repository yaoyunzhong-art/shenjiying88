...
function tierOrder(tier: MemberTier): number {
  const orderMap: Record<MemberTier, number> = {
    diamond: 5,
    gold: 4,
    silver: 3,
    bronze: 2,
...
function buildColumns(onRowClick: (item: MemberItem) => void): DataTableColumn<MemberItem>[] {
  return [
    {
      key: 'code',
      title: '会员编号',
    ...
    },
    {
      ...
      render: (item: MemberItem) => {
        const tierMeta = MEMBER_TIER_MAP[item.tier];
        return <StatusBadge label={tierMeta.label} variant={tierMeta.variant} size="sm" />;
      },
    },
    {
      key: 'points',
    ...
    },
    {
      ...
      render: (item: MemberItem) => {
        const statusMeta = MEMBER_STATUS_MAP[item.status];
...
export default function MembersClient({
  ...
  const { searchTerm, setSearchTerm, filteredItems } = useSearchFilter(snapshot.members, searchFields);

  const [tierFilter, setTierFilter] = useState<MemberTier | 'ALL'>('ALL');
  const tierFiltered = useMemo(
    () => (tierFilter === 'ALL' ? filteredItems : filteredItems.filter((item) => item.tier === tierFilter)),
    [filteredItems, tierFilter],
  );

  const [statusFilter, setStatusFilter] = useState<MemberStatus | 'ALL'>('ALL');
  const statusFiltered = useMemo(
    () => (statusFilter === 'ALL' ? tierFiltered : tierFiltered.filter((item) => item.status === statusFilter)),
    [tierFiltered, statusFilter],
  );

  const [storeFilter, setStoreFilter] = useState<string>('ALL');
  const storeFiltered = useMemo(
    () => (storeFilter === 'ALL' ? statusFiltered : statusFiltered.filter((item) => item.storeName === storeFilter)),
    [statusFiltered, storeFilter],
  );

  const [marketFilter, setMarketFilter] = useState<string>('ALL');
  const marketFiltered = useMemo(
    () => (marketFilter === 'ALL' ? storeFiltered : storeFiltered.filter((item) => item.marketCode === marketFilter)),
  ...
  const stats = useMemo(() => {
    ...
    const totalPoints = snapshot.members.reduce((sum, member) => sum + member.points, 0);
    const diamond = snapshot.members.filter((member) => member.tier === 'diamond').length;
    return { total: snapshot.members.length, active, totalPoints, diamond };
  }, [snapshot.members]);
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, marginBottom: 16 }}>
          <p style={{ margin: 0, color: '#94a3b8', fontSize: 13 }}>
            当前列表由服务端会员快照驱动，搜索、筛选与翻页保留在客户端渲染层。
          </p>
          <button
            type="button"
            onClick={() =>
        ...
        >
          <article style={statCardStyle}>
            ...
            <div style={{ marginTop: 4, fontSize: 12, color: '#94a3b8' }}>覆盖 {ALL_MARKETS.length} 个市场</div>
          </article>
          <article style={statCardStyle}>
          ...
          <article style={statCardStyle}>
            ...
            <div style={{ marginTop: 6, fontSize: 24, fontWeight: 700, color: '#a78bfa' }}>{stats.diamond}</div>
            <div style={{ marginTop: 4, fontSize: 12, color: '#94a3b8' }}>
        ...
        <div style={{ marginBottom: 12 }}>
          <Tabs
            items={[
              { key: 'ALL', label: '全部等级', count: snapshot.members.length },
              ...MEMBER_TIERS.map((tier) => ({
                key: tier,
                label: MEMBER_TIER_MAP[tier].label,
                count: snapshot.members.filter((item) => item.tier === tier).length,
              })),
            ]}
        ...
        >
            ...
            <Tabs
              items={[
                ...
                ...MEMBER_STATUSES.map((status) => ({
                  ...
                  count: tierFiltered.filter((item) => item.status === status).length,
                })),
              ]}
          ...
          <div>
            <div style={{ fontSize: 12, color: '#94a3b8', marginBottom: 6 }}>门店</div>
            <Tabs
              items={[
            ...
            <Tabs
              items={[
                ...
                ...ALL_MARKETS.map((market) => ({
                  key: market,
                  label: market,
                  count: storeFiltered.filter((item) => item.marketCode === market).length,
                })),
              ]}
              ...
              ? [
                  {
                    ...
                    count: tierFiltered.filter((item) => item.tier === tierFilter).length,
                  },
                ]
              : []),
            ...(statusFilter !== 'ALL'
              ...
              ? [
                  {
                    ...
                    count: statusFiltered.filter((item) => item.storeName === storeFilter).length,
                  },
                ]
              : []),
            ...(marketFilter !== 'ALL'
              ? [
                  {
                    ...
                    label: marketFilter,
                    tone: 'neutral' as FilterChip['tone'],
                    count: storeFiltered.filter((item) => item.marketCode === marketFilter).length,
                  },
                ]
              : []),
          ]}
