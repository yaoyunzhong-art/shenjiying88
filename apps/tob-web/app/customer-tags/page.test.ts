/**
 * customer-tags/page.test.ts — 客户画像标签管理页 (ToB) L2 纯源码测试
 *
 * 覆盖: 常量映射、Mock 数据、标签映射、筛选逻辑、统计计算、表单校验、CRUD、页面结构
 */
...
const MOCK_TAGS: Tag[] = [
  { id: 't1', name: '高净值企业', category: 'consumption', color: 'purple', source: 'ai-prediction', memberCount: 1243, description: '近6个月签约金额前5%', enabled: true, createdAt: '2025-12-01' },
  { id: 't2', name: '沉睡客户', category: 'behavior', color: 'orange', source: 'rule-engine', memberCount: 8720, description: '超过90天未登录', enabled: true, createdAt: '2025-11-15' },
  { id: 't3', name: '新兴企业', category: 'demographics', color: 'cyan', source: 'manual', memberCount: 5601, description: '成立3年以内', enabled: true, createdAt: '2025-10-20' },
  { id: 't4', name: 'SaaS关注者', category: 'lifestyle', color: 'pink', source: 'manual', memberCount: 3390, description: '近30天浏览SaaS产品', enabled: true, createdAt: '2025-12-10' },
  { id: 't5', name: '活动高参与', category: 'engagement', color: 'green', source: 'ai-prediction', memberCount: 2145, description: '月度活动参与率>60%', enabled: true, createdAt: '2025-09-05' },
  { id: 't6', name: '高流失风险', category: 'behavior', color: 'red', source: 'rule-engine', memberCount: 896, description: '流失预测分值>0.7', enabled: false, createdAt: '2025-08-12' },
  { id: 't7', name: '夜间活跃', category: 'lifestyle', color: 'blue', source: 'ai-prediction', memberCount: 4560, description: '主要登录时段22:00-02:00', enabled: true, createdAt: '2025-11-28' },
];
...
describe('customer-tags — filterTags', () => {
  ...
  it('22. 按名称搜索', () => { const r = filterTags(MOCK_TAGS, '沉睡', ''); assert.equal(r.length, 1); assert.equal(r[0]?.name, '沉睡客户'); });
  ...
  it('25. 组合过滤', () => { assert.equal(filterTags(MOCK_TAGS, '夜间', 'lifestyle').length, 1); });
...
describe('customer-tags — hooks验证', () => {
  ...
  it('65. 包含useState声明', () => assert.ok(src.includes('const [') && src.includes('useState')));
  it('66. 包含JSX返回', () => assert.ok(src.includes('return (') || src.includes('return <')));
  it('67. 包含事件处理器', () => assert.ok(src.includes('onClick={') || src.includes('onChange={')));
  it('68. 包含列表渲染', () => assert.ok(src.includes('.map(')));
  it('69. 包含条件渲染', () => assert.ok(src.includes(' && ') || src.includes(' ? ')));
  it('70. 包含样式定义', () => assert.ok(src.includes('style={')));
  it('71. 包含数据格式化(toLocaleString)', () => assert.ok(src.includes('toLocaleString')));
  it('72. 包含模板字符串', () => assert.ok(src.includes('${')));
  it('73. 包含默认导出', () => assert.ok(src.includes('export default function')));
  it('74. 包含注释说明', () => assert.ok(src.includes("/**") || src.includes('//')));
});
