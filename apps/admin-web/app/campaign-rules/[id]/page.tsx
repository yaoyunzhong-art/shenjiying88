...
/**
 * 营销活动管理详情页 — G4退回修复: 活动列表(+搜索+筛选) / 创建Modal / 编辑 / 状态管理
 */

import React, { useState, useMemo } from 'react';
import {
  Button, DataTable, DetailClosureBar, Modal, PageShell, Pagination, SearchFilterInput,
  Select, StatusBadge, Tabs, usePagination, useSortedItems,
...
const STATUS_MAP: Record<string, { label: string; variant: 'success' | 'warning' | 'default' }> = {
  draft: { label: '草稿', variant: 'default' }, active: { label: '进行中', variant: 'success' }, ended: { label: '已结束', variant: 'warning' },
};
...
function buildColumns(onClick: (item: CampaignActivity) => void): DataTableColumn<CampaignActivity>[] {
  return [
    ...
    { key: 'discount', title: '折扣', dataKey: 'discount', sortable: true, render: (item: CampaignActivity) => <span className="font-mono text-slate-300">{item.discount < 1 ? `${(item.discount * 100).toFixed(0)}折` : '无折扣'}</span> },
    { key: 'targetAudience', title: '目标人群', dataKey: 'targetAudience', sortable: true },
    { key: 'startDate', title: '开始日期', dataKey: 'startDate', sortable: true, render: (item: CampaignActivity) => <span className="text-xs text-slate-400">{item.startDate}</span> },
    { key: 'budget', title: '预算', dataKey: 'budget', sortable: true, align: 'right', render: (item: CampaignActivity) => <span className="font-mono text-emerald-400">{fmt(item.budget)}</span> },
    { key: 'hitCount', title: '参与数', dataKey: 'hitCount', sortable: true, align: 'right', render: (item: CampaignActivity) => <span className="font-mono text-slate-300">{item.hitCount.toLocaleString()}</span> },
  ];
}

function Field({ label, required, error, children }: { label: string; required?: boolean; error?: string; children: React.ReactNode }) {
  return <div><label className="block text-xs text-slate-400 mb-1">{label}{required && ' *'}</label>{children}{error && <p className="text-xs text-red-400 mt-1">{error}</p>}</div>;
}
...
function CreateModal({ open, onClose, onCreate }: { open: boolean; onClose: () => void; onCreate: (a: CampaignActivity) => void }) {
  ...
  return <Modal open={open} onClose={onClose} title="创建营销活动"><div className="space-y-4">
    <Field label="活动名称" required error={err.name}><input className={inputClass} value={n} onChange={(e) => setN(e.target.value)} placeholder="输入活动名称" /></Field>
    <Field label="活动描述"><input className={inputClass} value={d} onChange={(e) => setD(e.target.value)} placeholder="输入活动描述" /></Field>
    <div className="grid grid-cols-2 gap-4">
      <Field label="开始日期" required error={err.start}><input type="date" className={inputClass} value={s} onChange={(e) => setS(e.target.value)} /></Field>
      <Field label="结束日期" required error={err.end}><input type="date" className={inputClass} value={e} onChange={(e) => setE(e.target.value)} /></Field>
    </div>
    <div className="grid grid-cols-2 gap-4">
      <Field label="折扣率(%)" error={err.discount}><input type="number" className={inputClass} value={dc} onChange={(e) => setDc(e.target.value)} min={1} max={100} /></Field>
      <Field label="目标人群" required error={err.target}>
        <select className={inputClass} value={t} onChange={(e) => setT(e.target.value)}><option value="">请选择</option>{TARGET_OPTS.slice(1).map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}</select>
      </Field>
    </div>
...
function EditPanel({ activity, onClose, onSave, onDelete, onStatusChange }: {
  ...
  const [dc, setDc] = useState(String(activity.discount * 100)); const [t, setT] = useState(activity.targetAudience);
  const [bg, setBg] = useState(String(activity.budget)); const [sv, setSv] = useState(false);
  const handleSave = async () => { setSv(true); await new Promise((r) => setTimeout(r, 300)); onSave({ ...activity, name: n, description: d, discount: Number(dc) / 100, targetAudience: t, budget: Number(bg) }); setSv(false); };
  return <div className="fixed inset-y-0 right-0 w-[420px] bg-slate-900 border-l border-slate-700 shadow-2xl z-50 overflow-y-auto p-6">
    <div className="flex items-center justify-between mb-6"><h2 className="text-lg font-semibold text-white">编辑活动</h2><button onClick={onClose} className="text-slate-400 hover:text-white text-xl">✕</button></div>
    ...
    <div className="mt-6 pt-4 border-t border-slate-700">
      ...
      <div className="flex gap-2 mb-4">
        {(['draft', 'active', 'ended'] as const).map((st) => (
          <button key={st} disabled={activity.status === st} onClick={() => onStatusChange(activity.id, st)}
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activity.status === st ? 'bg-blue-600 text-white cursor-default' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}>{STATUS_MAP[st]?.label ?? st}</button>
        ))}
      </div>
...
export default function CampaignRuleDetailPage() {
  ...
  const [search, setSearch] = useState(''); const [sf, setSf] = useState(''); const [tf, setTf] = useState('');
  ...
  const st = useMemo(() => ({ t: acts.length, d: acts.filter((a) => a.status === 'draft').length, a: acts.filter((a) => a.status === 'active').length, e: acts.filter((a) => a.status === 'ended').length }), [acts]);
  ...
  return <PageShell title="营销活动管理" description="管理全部营销活动，支持创建、编辑与状态切换">
    <div className="flex gap-3 mb-4 flex-wrap">
      {[{ l: '活动总数', v: st.t, c: 'text-blue-400' }, { l: '进行中', v: st.a, c: 'text-emerald-400' }, { l: '草稿', v: st.d, c: 'text-slate-400' }, { l: '已结束', v: st.e, c: 'text-amber-400' }, { l: '总预算', v: fmt(acts.reduce((s, a) => s + a.budget, 0)), c: 'text-violet-400' }].map((card) => (
    ...
    <Tabs items={[{ key: 'all', label: '全部', count: st.t }, { key: 'active', label: '进行中', count: st.a }, { key: 'draft', label: '草稿', count: st.d }, { key: 'ended', label: '已结束', count: st.e }]} activeKey={tab} onChange={(k) => { setTab(k); pg.setPage(1); setSearch(''); setSf(''); setTf(''); }} />
    ...
    {editAct && <EditPanel activity={editAct} onClose={() => setEditAct(null)} onSave={(u) => { setActs((prev) => prev.map((x) => x.id === u.id ? u : x)); setEditAct(null); }} onDelete={(id) => { setActs((prev) => prev.filter((x) => x.id !== id)); setEditAct(null); }} onStatusChange={(id, s) => setActs((prev) => prev.map((x) => x.id === id ? { ...x, status: s } : x))} />}
