...
const MOCK: CampaignActivity[] = [
  { id: 'act-001', name: '618 年中大促', description: '年中购物节全场折扣',
    startDate: '2026-06-01', endDate: '2026-06-20', status: 'ended',
    discount: 0.7, targetAudience: '全部会员', budget: 500000, hitCount: 12430 },
  { id: 'act-002', name: '新会员首单礼', description: '新注册会员首单专享',
    startDate: '2026-07-01', endDate: '2026-08-31', status: 'active',
    discount: 0.8, targetAudience: '新注册会员', budget: 200000, hitCount: 3456 },
  { id: 'act-003', name: '夏季清凉特卖', description: '夏季商品清仓促销',
    startDate: '2026-07-15', endDate: '2026-08-15', status: 'draft',
    discount: 0.6, targetAudience: '高活跃会员', budget: 300000, hitCount: 0 },
  { id: 'act-004', name: '会员日双倍积分', description: '每月15日会员日双倍积分',
    startDate: '2026-07-15', endDate: '2026-12-31', status: 'active',
    discount: 1, targetAudience: '全部会员', budget: 100000, hitCount: 5678 },
  { id: 'act-005', name: '开学季大促', description: '开学季文具电子类促销',
    startDate: '2026-08-20', endDate: '2026-09-10', status: 'draft',
    discount: 0.75, targetAudience: '学生认证会员', budget: 250000, hitCount: 0 },
  { id: 'act-006', name: '双11 预热', description: '双十一提前预售活动',
    startDate: '2026-10-20', endDate: '2026-11-11', status: 'draft',
    discount: 0.5, targetAudience: '全部会员', budget: 1000000, hitCount: 0 },
];
...
function buildColumns(
  onClick: (item: CampaignActivity) => void,
): DataTableColumn<CampaignActivity>[] {
  return [
    ...
    { key: 'discount', title: '折扣', dataKey: 'discount', sortable: true,
      render: (item: CampaignActivity) => (
        <span className="font-mono text-slate-300">
          {item.discount < 1 ? `${(item.discount * 100).toFixed(0)}折` : '无折扣'}
        </span>
      ),
    },
...
function CreateModal({ open, onClose, onCreate }: {
  ...
  const handleCreate = async () => {
    ...
    onCreate({
      id: `act-${Date.now().toString(36)}`, name: n.trim(), description: d.trim(),
      startDate: s, endDate: e, status: 'draft',
      discount: Number(dc) / 100, targetAudience: t, budget: Number(bg), hitCount: 0,
    });
    ...
    <Modal open={open} onClose={onClose} title="创建营销活动">
      <div className="space-y-4">
        ...
        <div className="flex gap-3 justify-end">
          ...
          <Button variant="primary" disabled={sv} onClick={handleCreate}>
            {sv ? '创建中...' : '创建活动'}
          </Button>
        </div>
      </div>
    ...
    <div className="fixed inset-y-0 right-0 w-[420px] bg-slate-900 border-l border-slate-700 shadow-2xl z-50 overflow-y-auto p-6">
      ...
      <div className="space-y-4">
        ...
        <div className="flex gap-3 justify-end pt-2">
          <Button variant="secondary" onClick={onClose}>取消</Button>
          <Button variant="primary" disabled={sv} onClick={handleSave}>{sv ? '保存中...' : '保存'}</Button>
        </div>
      </div>
...
export default function CampaignRuleDetailPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (sf) items = items.filter((a) => a.status === sf);
    if (tf) items = items.filter((a) => a.targetAudience === tf);
    return items;
    ...
    <PageShell title="营销活动管理" description="管理全部营销活动，支持创建、编辑与状态切换">
      ...
      <div className="flex items-center gap-3 mb-3 flex-wrap">
        <div className="flex-1 min-w-[180px]">
          <SearchFilterInput placeholder="搜索活动名称/描述..." value={search} onChange={(v) => { setSearch(v); pg.setPage(1); }} />
        </div>
      ...
      <div className="flex justify-end mt-4">
        <Pagination page={pg.page} pageSize={pg.pageSize} total={sorted.length} onPageChange={pg.setPage} onPageSizeChange={pg.setPageSize} />
      </div>
