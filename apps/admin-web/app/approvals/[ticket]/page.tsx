...
const permissionGate = {
  ...
  title: '审批详情访问受限',
  description: '该页面已接入管理员权限管控，仅具备 foundation.governance.read 权限的账号可访问。',
} as const
...
export default async function ApprovalDetailPage({ params }: PageProps) {
  ...
  const snapshot = await loadApprovalDetailSnapshot(ticket)
  ...
  return (
    <AdminPermissionGate {...permissionGate}>
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: 24 }}>
        ...
        <ApprovalDetailClient snapshot={snapshot} />
      </div>
    </AdminPermissionGate>
  )
}
