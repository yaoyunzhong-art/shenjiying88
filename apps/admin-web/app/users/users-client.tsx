...
const MOCK_USERS: User[] = [
  { id: 'u1', name: '张三', email: 'zhangsan@m5.com', role: 'super_admin', permissions: ['*', 'identity-access:write', 'user:write'], status: 'active', store: '总部', lastLogin: '2026-07-17 09:15', createdAt: '2024-01-01' },
  { id: 'u2', name: '李四', email: 'lisi@m5.com', role: 'store_manager', permissions: ['store:read', 'staff:read', 'staff:write'], status: 'active', store: '朝阳店', lastLogin: '2026-07-17 08:30', createdAt: '2024-03-15' },
  { id: 'u3', name: '王五', email: 'wangwu@m5.com', role: 'staff', permissions: ['store:read', 'schedule:read'], status: 'active', store: '朝阳店', lastLogin: '2026-07-16 18:45', createdAt: '2024-06-01' },
  { id: 'u4', name: '赵六', email: 'zhaoliu@m5.com', role: 'finance', permissions: ['finance:read', 'finance:export'], status: 'active', store: '总部', lastLogin: '2026-07-17 10:00', createdAt: '2024-05-20' },
  { id: 'u5', name: '孙七', email: 'sunqi@m5.com', role: 'marketing', permissions: ['campaign:read', 'campaign:write'], status: 'active', store: '总部', lastLogin: '2026-07-16 16:20', createdAt: '2024-07-01' },
  { id: 'u6', name: '周八', email: 'zhouba@m5.com', role: 'staff', permissions: ['store:read'], status: 'inactive', store: '朝阳店', lastLogin: '2026-06-30 12:00', createdAt: '2025-01-10' },
  { id: 'u7', name: '吴九', email: 'wujiu@m5.com', role: 'ops', permissions: ['monitor:read', 'alert:write'], status: 'active', store: '总部', lastLogin: '2026-07-17 07:50', createdAt: '2024-09-01' },
  { id: 'u8', name: '郑十', email: 'zhengshi@m5.com', role: 'store_manager', permissions: ['store:read', 'staff:read'], status: 'suspended', store: '海淀店', lastLogin: '2026-07-10 14:30', createdAt: '2024-11-01' },
];
...
export default function UsersClient() {
  ...
  const filtered = useMemo(() => {
    return MOCK_USERS.filter(u => {
      const matchesSearch = !searchTerm || `${u.name} ${u.email} ${u.store} ${u.permissions.join(' ')}`.toLowerCase().includes(searchTerm.toLowerCase());
      ...
      render: (item) => (
        <div style={{ display: 'grid', gap: 2 }}>
          ...
          <span style={{ fontSize: 12, color: '#64748b' }}>
            {item.permissions.includes('*') ? '全部权限' : `权限 ${item.permissions.length} 项`}
          </span>
        </div>
      ),
    },
