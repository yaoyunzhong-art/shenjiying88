...
export interface KnowledgeSnapshotDelivery {
  deliveryMode: 'mock';
...
function getLatestKnowledgeTimestamp(data: KnowledgeSnapshot): string {
  const values = [
    ...data.categories.map((item) => item.lastUpdated),
    ...data.recentDocuments.map((item) => item.updatedAt),
  ];
  return values.reduce((latest, value) => (value > latest ? value : latest), '—');
}

export async function loadKnowledgeSnapshot(): Promise<KnowledgeSnapshotDelivery> {
  const data = await loadKnowledge();
  return {
    deliveryMode: 'mock',
    data,
    generatedAt: getLatestKnowledgeTimestamp(data),
  };
}
