...
import type { AdminWorkbenchConsumerSnapshot } from '../bootstrap';
...
interface PadIndexClientProps {
  snapshot: AdminWorkbenchConsumerSnapshot;
}

export default function PadIndexClient({ snapshot }: PadIndexClientProps) {
  const [searchQuery, setSearchQuery] = useState('');
  const [categoryFilter, setCategoryFilter] = useState('all');
  const [selectedWorkbench, setSelectedWorkbench] = useState<RoleWorkbenchContract | null>(null);

  const allPadWorkbenches = useMemo(
    () => filterPadWorkbenches(snapshot.workbenches),
  ...
  const sourceEvidence = useMemo(
    () => ({
      ...
      note:
        snapshot.deliveryMode === 'api'
          ? 'Pad 目录已接入 bootstrap snapshot，当前展示的是实时工作台快照。'
          : 'Pad 目录当前仍使用静态回退清单，待工作台 bootstrap 恢复后将自动切回 API 实时来源态。',
    }),
  ...
  const handleRefresh = useCallback(() => {
    // Server wrapper 已接 bootstrap snapshot，这里保留按钮以对齐目录页交互。
  }, []);
  ...
  return (
    <main style={{ maxWidth: 1024, margin: '0 auto', padding: 20 }}>
      <AdminPermissionGate
        ...
        title="Pad 工作台访问受限"
        description="Pad 工作台目录已接入管理员本地 session，只有具备 workbench.read 的账号才能查看现场角色入口。"
      >
        <PageShell
          title="Pad 工作台"
