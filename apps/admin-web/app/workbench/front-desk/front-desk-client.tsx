...
interface FrontDeskWorkbenchClientProps {
...
export default function FrontDeskWorkbenchClient({
  deliveryMode,
  roleWorkbench,
}: FrontDeskWorkbenchClientProps) {
  ...
  const backendRole = mapToBackendRole(roleWorkbench?.role ?? 'CASHIER');
  ...
  const sourceEvidence = useMemo(
    () => ({
      ...
      businessDataSource: 'MOCK_PRODUCT_SUGGESTIONS + local basket/queue/todayStats',
      note:
        deliveryMode === 'api'
          ? '前台工作台壳层已接入 bootstrap snapshot，但购物篮、排队、商品搜索与营业指标仍为本地 mock 样本。'
          : '前台工作台壳层当前回退到 fallbackRoleWorkbenches，购物篮、排队、商品搜索与营业指标仍为本地 mock 样本。',
    }),
    [deliveryMode],
  );
  ...
  const handleCallNext = useCallback(() => {
    setQueue((prev) => {
      const next = prev.find((item) => item.status === 'waiting');
      if (!next) return prev;
      return prev.map((item) =>
        item.id === next.id ? { ...item, status: 'calling' as const } : item,
      );
    });
  }, []);
  ...
  const quickActions: QuickFnButton[] = useMemo(
    () => [
      ...
      },
      {
        ...
        label: '叫号',
        icon: '🔔',
        badge: queue.filter((item) => item.status === 'waiting').length,
  ...
  // E54 控制面证据：前台工作台已接入角色快照，但收银业务面仍是 mock 状态。
  return (
    <AdminPermissionGate {...permissionGate}>
      <PageShell
        title="前台操作面板"
        subtitle={roleWorkbench?.description ?? '前台收银 / 排队叫号 / 快捷操作'}
        actions={
          <DetailActionBar
            actions={[
      ...
      >
                  ...
                  >
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <div
                        style={{
                          fontSize: 11,
                          color: product.stock > 30 ? '#22c55e' : '#f59e0b',
                        }}
                      >
                        库存: {product.stock}
                      </div>
        ...
        <FrontDeskPanel
          title="收银台"
          cashierName={roleWorkbench?.title ?? '王收银'}
                  ...
                  >
                    <span
                      style={{
                        fontSize: 16,
                        fontWeight: 700,
                        color: item.status === 'calling' ? '#22c55e' : '#e2e8f0',
                      }}
                    >
                      {item.number}
                    </span>
                    <div style={{ fontSize: 12, color: '#94a3b8' }}>
                      {item.customerName ?? '—'} {item.waitingMinutes}min
                    </div>
                    {item.status === 'calling' && (
                      <button
                        onClick={() => handleCompleteServing(item.id)}
                        style={{
                          marginLeft: 'auto',
                          padding: '4px 10px',
                          borderRadius: 6,
