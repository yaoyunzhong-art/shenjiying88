...
const permissionGate = {
  ...
  title: '会员等级详情访问受限',
  description: '会员等级详情页已接入管理员本地 session，只有具备 member:read 的账号才能查看等级规则、状态流转与编辑信息。',
} as const;

export const dynamic = 'force-dynamic';
export const revalidate = 0;

export default async function MemberTierDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <AdminPermissionGate {...permissionGate}>
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: 24 }}>
        ...
        <MemberTierDetailClient snapshot={snapshot} />
      </div>
    </AdminPermissionGate>
  );
}
