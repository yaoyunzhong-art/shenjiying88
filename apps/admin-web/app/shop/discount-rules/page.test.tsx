...
function setup() {
  ...
  window.localStorage.setItem(
    ...
    JSON.stringify({
      userId: 'admin:test',
      role: 'super-admin',
      permissions: ['shop:discount-rules:read'],
    }),
  );
...
describe('discount-rules: 页面渲染', () => {
  it('源码包含页面标题', () => { assert.ok(SRC.includes('折扣规则')); });
  it('源码包含页面描述', () => { assert.ok(SRC.includes('折扣')); });
  it('源码包含页面主布局', () => { assert.ok(SRC.includes('PageShell') || SRC.includes('return (')); });
...
describe('Shop / Discount Rules — hooks验证', () => {
  it('应接入管理员权限边界', () => {
    assert.ok(SRC.includes('AdminPermissionGate'));
    assert.ok(SRC.includes("requiredPermission: 'shop:discount-rules:read'"));
  });
