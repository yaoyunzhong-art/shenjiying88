...
function setup() {
  ...
  window.localStorage.setItem(
    ...
    JSON.stringify({
      userId: 'admin:test',
      role: 'super-admin',
      permissions: ['shop:order-reviews:read'],
    }),
  );
...
describe('order-reviews: 页面渲染', () => {
  it('源码包含页面标题', () => { assert.ok(SRC.includes('订单评价')); });
  it('源码包含页面描述', () => { assert.ok(SRC.includes('评价')); });
...
describe('Shop / Order Reviews — hooks验证', () => {
  it('应接入管理员权限边界', () => {
    assert.ok(SRC.includes('AdminPermissionGate'));
    assert.ok(SRC.includes("requiredPermission: 'shop:order-reviews:read'"));
  });
