...
function setup() {
  ...
  window.localStorage.setItem(
    ...
    JSON.stringify({
      userId: 'admin:test',
      role: 'super-admin',
      permissions: ['shop:fulfillment:read'],
    }),
  );
...
describe('fulfillment: 页面渲染', () => {
  it('源码包含页面标题', () => { assert.ok(SRC.includes('履约管理')); });
  it('源码包含页面描述', () => { assert.ok(SRC.includes('履约')); });
...
describe('Shop / Fulfillment — hooks验证', () => {
  it('应接入管理员权限边界', () => {
    ...
    assert.ok(SRC.includes("requiredPermission: 'shop:fulfillment:read'"));
  });
