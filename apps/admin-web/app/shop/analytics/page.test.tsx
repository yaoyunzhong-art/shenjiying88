...
function setup() {
  ...
  window.localStorage.setItem(
    ...
    JSON.stringify({
      userId: 'admin:test',
      role: 'super-admin',
      permissions: ['shop:analytics:read'],
    }),
  );
...
describe('shop-analytics: 页面渲染', () => {
  it('源码包含页面标题', () => { assert.ok(SRC.includes('店铺数据分析')); });
  it('源码包含页面描述', () => { assert.ok(SRC.includes('分析')); });
...
describe('Shop / Analytics — hooks验证', () => {
  it('应接入管理员权限边界', () => {
    assert.ok(SRC.includes('AdminPermissionGate'));
    assert.ok(SRC.includes("requiredPermission: 'shop:analytics:read'"));
  });
