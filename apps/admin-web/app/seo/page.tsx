...
const permissionGate = {
  ...
  title: 'SEO 工作台访问受限',
  description:
    'SEO 工作台已接入管理员本地 session，只有具备 dashboard:read 的账号才能查看健康度、SEO 模块和优化建议。',
} as const;
...
export default async function SeoPage({ searchParams }: PageProps) {
  ...
  const snapshot = await loadSeoSnapshot(resolveTenantId(query.tenantId));
