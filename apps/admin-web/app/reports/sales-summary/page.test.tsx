...
describe('sales-summary: 页面渲染', () => {
  it('包含页面标题', () => {
    assert.ok(SRC.includes('销售汇总报表'));
  });

  it('包含页面说明', () => {
    assert.ok(SRC.includes('每日销售明细'));
  });

  it('包含门禁标题与说明', () => {
    assert.ok(SRC.includes('销售汇总访问受限'));
...
describe('Reports / Sales Summary — hooks验证', () => {
  ...
  it('包含注释说明', () => assert.ok(SRC.includes("/**") || SRC.includes('//') || SRC.includes("'use client'")));
});
