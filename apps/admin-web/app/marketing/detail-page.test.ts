/**
 * marketing/detail-page.test.ts — Marketing campaign detail lookups & utilities.
 * Tests campaignStatusLabel, campaignChannelLabel, canTransition, validateCampaignForm,
 * and getMarketingCampaignById from the detail utilities module.
 *
...
describe('marketing-detail: 边界', () => {
  ...
  describe('getMarketingCampaignById — mock 数据完整性', () => {
    it('all campaigns should have valid status', () => {
    ...
    it('all campaigns should have valid channel', () => {
      const ids = ['c1', 'c2', 'c3', 'c4', 'c5'];
      for (const id of ids) {
        const c = getMarketingCampaignById(id);
        assert.ok(c, `${id} should exist`);
    ...
    it('all campaigns should have non-empty name', () => {
      const ids = ['c1', 'c2', 'c3', 'c4', 'c5'];
      for (const id of ids) {
        const c = getMarketingCampaignById(id);
        assert.ok(c, `${id} should exist`);
    ...
    it('all campaigns should have startAt date', () => {
      const ids = ['c1', 'c2', 'c3', 'c4', 'c5'];
      for (const id of ids) {
        const c = getMarketingCampaignById(id);
        assert.ok(c, `${id} should exist`);
    ...
    it('cost should be >= 0 for all campaigns', () => {
      const ids = ['c1', 'c2', 'c3', 'c4', 'c5'];
      for (const id of ids) {
        const c = getMarketingCampaignById(id);
        assert.ok(c, `${id} should exist`);
    ...
    it('draft campaigns should have reachCount=0 and conversionRate=0', () => {
      const ids = ['c4', 'c5'];
      for (const id of ids) {
        const c = getMarketingCampaignById(id);
        assert.strictEqual(c!.status, 'draft');
    ...
    it('ended campaign (c2) should have endAt defined', () => {
      const c = getMarketingCampaignById('c2');
      assert.strictEqual(c!.status, 'ended');
      assert.ok(c!.endAt, 'ended campaign should have endAt');
    });

    it('scheduled campaign (c3) should have reachCount configured', () => {
      const c = getMarketingCampaignById('c3');
      assert.strictEqual(c!.status, 'scheduled');
