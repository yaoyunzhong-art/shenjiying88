...
export default function CampaignPerformanceClient({ snapshot }: { snapshot: MarketingPerformanceSnapshot }) {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      try {
        const data = generateMockAnalytics(id);
        setAnalytics(data);
      } catch {
        setError('加载活动分析数据失败');
      } finally {
  ...
  const crumbs: Array<{ label: string; href?: string }> = [
    ...
    { label: analytics.campaignName, href: `/marketing/${id}` },
    { label: '性能分析' },
  ];
