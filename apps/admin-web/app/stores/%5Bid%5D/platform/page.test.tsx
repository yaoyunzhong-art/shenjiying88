...
const CLIENT_SRC = readFileSync(resolve(DIR, 'platform-client.tsx'), 'utf-8')
const DATA_SRC = readFileSync(resolve(DIR, 'platform-data.ts'), 'utf-8')

describe('stores/%5Bid%5D/platform 结构固证', () => {
  it('page 应切为 server wrapper 并加载快照', () => {
    assert.ok(!PAGE_SRC.includes("'use client'"))
    assert.ok(PAGE_SRC.includes('export const dynamic = \'force-dynamic\''))
    assert.ok(PAGE_SRC.includes('export const revalidate = 0'))
    assert.ok(PAGE_SRC.includes('export default async function PlatformPage'))
    assert.ok(PAGE_SRC.includes('const snapshot = await loadPlatformSnapshot()'))
  ...
  it('data 应定义 mock 快照合同', () => {
    ...
    assert.ok(DATA_SRC.includes('loadPlatformSnapshot'))
  })
})
