...
describe('stores/[id]/scheduling/page.tsx 结构固证', () => {
  it('page 应保持最小 server wrapper 并桥接快照到 client', () => {
    ...
    assert.ok(PAGE_SRC.includes('const snapshot = await loadSchedulingSnapshot'))
    assert.ok(PAGE_SRC.includes('<SchedulingClient snapshot={snapshot} />'))
