...
describe('stores/[id]/orders/page.tsx 结构固证', () => {
  it('page 应保持最小 server wrapper 并桥接快照到 client', () => {
    assert.ok(!PAGE_SRC.includes("'use client'"))
    assert.ok(PAGE_SRC.includes('export default async function OrdersPage'))
    assert.ok(PAGE_SRC.includes('const snapshot = await loadStoreOrdersSnapshot'))
    assert.ok(PAGE_SRC.includes('<StoreOrdersClient snapshot={snapshot} />'))
