...
describe('stores/[id]/tenant/page.tsx 结构固证', () => {
  it('page 应保持最小 server wrapper 并桥接快照到 client', () => {
    assert.ok(!PAGE_SRC.includes("'use client'"))
    assert.ok(PAGE_SRC.includes('export default async function TenantPage'))
    assert.ok(PAGE_SRC.includes('const snapshot = await loadTenantSnapshot'))
