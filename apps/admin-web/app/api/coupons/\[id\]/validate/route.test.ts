/**
 * api/coupons/[id]/validate/route.test.ts — 优惠券验证代理层 L1 测试
 *
 * 覆盖: 正例·边界·防御
 */
...
describe.skip('coupons/[id]/validate — 正例', () => {
  it('应导出 POST 方法验证优惠券', () => {
  ...
  it('应包含 getValidateApi 函数构建验证 URL', () => {
    assert.ok(SRC.includes('getValidateApi'), '缺少 getValidateApi');
    assert.ok(SRC.includes('params.id'), '应使用 params.id');
    assert.ok(SRC.includes('/validate'), '应包含 validate 路径');
  });
});

describe.skip('coupons/[id]/validate — 防御', () => {
  it('应使用 createProxyHandler 代理', () => {
    assert.ok(SRC.includes('createProxyHandler(getValidateApi('), '应使用代理');
  });
