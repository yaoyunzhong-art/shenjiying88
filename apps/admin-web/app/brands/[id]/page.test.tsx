...
describe('brands/[id] 结构固证', () => {
  it('page 应迁移为服务端 wrapper', () => {
    assert.ok(!PAGE_SRC.includes("'use client'"))
    assert.ok(PAGE_SRC.includes('export default async function BrandDetailPage'))
    assert.ok(PAGE_SRC.includes('const snapshot = await loadBrandDetailSnapshot(id)'))
    assert.ok(PAGE_SRC.includes('<BrandDetailClient snapshot={snapshot} />'))
  })
  ...
  it('data 应定义 fallback 详情快照合同', () => {
    ...
    assert.ok(DATA_SRC.includes("sourceLabel: 'brand-detail-fallback'"))
    assert.ok(DATA_SRC.includes('id: string'))
    assert.ok(DATA_SRC.includes('brand-detail-legacy params bridge'))
  })
  ...
  it('legacy 应保留编辑、统计与收口动作', () => {
    assert.ok(LEGACY_SRC.includes('BRAND_STATUS_MAP'))
