...
describe('🔗 Chain35: P-30 后勤管理 — logistics/logistics-supplement/stock-transfer 验收', () => {
  ...
  describe('Logistics 物流管理', () => {
    ...
    it('L35-06 [B] 物流订单空items → 400', async () => {
      const res = await fetch('/api/logistics/orders', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          tenantId: 't001', origin: 'A', destination: 'B', items: [],
        }),
      })
  ...
  // ════════════════════════════════════════════════════════
  //  2. Logistics-Supplement — 物流补充
  // ════════════════════════════════════════════════════════

  describe('Logistics-Supplement 物流补充', () => {
    it('L35-07 [P] 创建运输调度单 — POST /logistics-supplement/transport-orders → 201', async () => {
      const res = await fetch('/api/logistics-supplement/transport-orders', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          tenantId: 't001', orderNumber: 'TO-001', transportType: 'normal',
          origin: '上海仓', destination: '杭州市西湖区',
          items: [{ cargoId: 'c1', cargoName: '设备', quantity: 3, unit: '台', weightKg: 150 }],
        }),
      })
      expect(res.status).toBe(201)
      const body = await res.json()
    ...
    it('L35-08 [P] 运输单状态流转: draft→dispatched→in_transit→completed', async () => {
    ...
    it('L35-09 [P] 添加货物装载 — POST /logistics-supplement/cargo-loads → 201', async () => {
    ...
    it('L35-10 [P] 路线规划 + 优化 — POST + POST /optimize → 优化后距离减少', async () => {
  ...
  // ════════════════════════════════════════════════════════
  //  3. Stock-Transfer — 库存调拨
  // ════════════════════════════════════════════════════════

  describe('Stock-Transfer 库存调拨', () => {
    it('L35-12 [P] 创建调拨单 — POST /stock-transfer → 201 + pending状态', async () => {
    ...
    it('L35-13 [P] 调拨全生命周期: pending→approved→in_transit→received', async () => {
    ...
    it('L35-14 [P] 调拨统计 — GET /stock-transfer/stats/:tenantId → 汇总数据', async () => {
    ...
    it('L35-15 [B] 空 items 创建 → 400', async () => {
      const res = await fetch('/api/stock-transfer', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    it('L35-16 [B] 已完成调拨不可取消 → 400', async () => {
      const create = await fetch('/api/stock-transfer', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  describe('跨模块集成', () => {
    it('L35-17 [P] 物流订单→补充运输→调拨 串联', async () => {
      // 1. 物流订单
