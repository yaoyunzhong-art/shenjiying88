...
it('E2E链#4 正例: 同一API bootstrap → 多端 snapshot 核心数据一致', () => {
...
it('E2E链#4 正例: admin/tob/storefront 三端从同一 portal 正确解析各自 role', () => {
  const portal = createPortalBootstrapResponse('cn-mainland');
...
it('E2E链#4 边界: consumer 消费顺序应保持 market → portal → workbench', () => {
...
it('E2E链#4 边界: supportedSurfaces 6端 在 store portal 中全部声明', () => {
...
it('E2E链#4 反例: fallback snapshot 机制 - API 不可用时的退化', () => {
  // 模拟 API 失败的场景
...
it('E2E链#4 跨市场: 中美两市场 bootstrap 多端产出不串台', () => {
...
it('E2E链#4 工作台: 5个角色各有专属 workbench', () => {
