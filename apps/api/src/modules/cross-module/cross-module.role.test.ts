...
describe(`${ROLES.TenantAdmin} cross-module 角色测试`, () => {
  ...
  test('店长执行全部链路验证 — 返回 4 条结果且全部通过', async () => {
  ...
  test('店长验证后 all-verified 应为 true', async () => {
  ...
  test('店长验证后 has-broken 应为 false', async () => {
    await ctrl.validate({ chainNames: undefined, ...defaultCtx })
    const broken = ctrl.getHasBroken()
    assert.equal(broken.hasBroken, false)
  })

  test('店长重置所有链路 — 状态回 Defined', async () => {
    await ctrl.validate({ chainNames: undefined, ...defaultCtx })
    ctrl.resetAll()
    const result = ctrl.getChainStatus()
  ...
  test('店长验证单条链路 admin-to-consumer — 返回 5 个阶段', async () => {
...
describe(`${ROLES.FrontDesk} cross-module 角色测试`, () => {
  ...
  test('前台可以查看跨模块链路状态 — 基本只读访问', () => {
    const result = ctrl.getChainStatus()
    assert.equal(result.total, 4)
    assert.ok(result.chains.length === 4)
  })

  test('前台获取验证摘要 — 摘要正常返回', () => {
    const summary = ctrl.getSummary()
    assert.ok(summary !== null)
    assert.ok(summary !== undefined)
  })

  test('前台检查所有链路验证状态 — all-verified 接口可访问', () => {
    const verified = ctrl.getAllVerified()
    assert.ok('allVerified' in verified)
    assert.ok('checkedAt' in verified)
  })

  test('前台不能执行重置操作 — 权限边界：前台应无法 reset', () => {
...
describe(`${ROLES.Hr} cross-module 角色测试`, () => {
  ...
  test('HR 可以查看跨模块链路状态 — 人员编制审计', () => {
    const result = ctrl.getChainStatus()
    assert.equal(result.total, 4)
  })

  test('HR 查看 governance-chain — 包含 5 个治理模块', () => {
    const result = ctrl.getChainStatus()
  ...
  test('HR 获取验证摘要 — 跨模块状态可审计', () => {
    const summary = ctrl.getSummary()
    assert.equal(summary.total, 4)
  })

  test('HR 查看 multi-client-consistency 链路 — 覆盖 5 客户端', () => {
    const result = ctrl.getChainStatus()
    const chain = result.chains.find(
...
describe(`${ROLES.Security} cross-module 角色测试`, () => {
  ...
  test('安监审计 cross-module — governance-chain 存在且状态 defined', () => {
    const result = ctrl.getChainStatus()
    const chain = result.chains.find(
      (c: { name: string; modules: string[]; status: string }) => c.name === 'governance-chain'
    )
  ...
  test('安监执行全部验证 — 验证后检查有无断开链路', async () => {
    await ctrl.validate({ chainNames: undefined, ...defaultCtx })
    const broken = ctrl.getHasBroken()
    assert.equal(broken.hasBroken, false)
  })

  test('安监重置后 — 所有链路回到 defined 且无 brokenNodes', () => {
    ctrl.resetAll()
    const result = ctrl.getChainStatus()
    for (const chain of result.chains) {
      assert.equal(chain.status, 'defined')
  ...
  test('安监验证后 all-verified 为 true — 安全基线已达标', async () => {
    await ctrl.validate({ chainNames: undefined, ...defaultCtx })
    const verified = ctrl.getAllVerified()
    assert.equal(verified.allVerified, true)
  })

  test('安监跨市场验证 — us-default 上下文不影响验证结果', async () => {
...
describe(`${ROLES.Guide} cross-module 角色测试`, () => {
  ...
  test('导玩员查看 — 所有 4 条链路状态可读', () => {
    const result = ctrl.getChainStatus()
    assert.equal(result.total, 4)
    assert.equal(result.chains.length, 4)
  })

  test('导玩员查看 admin-to-consumer — 终点为 miniapp（C 端入口）', () => {
    const result = ctrl.getChainStatus()
    const chain = result.chains.find(
  ...
  test('导玩员获取验证摘要 — 确认跨模块链路总数', () => {
    const summary = ctrl.getSummary()
    assert.equal(summary.total, 4)
  })

  test('导玩员查看 sdk-to-api — member 为终端模块', () => {
    const result = ctrl.getChainStatus()
    const chain = result.chains.find(
...
describe(`${ROLES.Operations} cross-module 角色测试`, () => {
  ...
  test('运行专员获取链路概要 — total > 0', () => {
    const result = ctrl.getChainStatus()
    assert.ok(result.total > 0)
    assert.equal(result.runtime, 'cross-module-e2e')
  })

  test('运行专员执行验证单条链路 — sdk-to-api 4 模块 3 阶段', async () => {
  ...
  test('运行专员验证后检查 — all-verified 在全部验证后为 true', async () => {
    await ctrl.validate({ chainNames: undefined, ...defaultCtx })
    const verified = ctrl.getAllVerified()
    assert.equal(verified.allVerified, true)
  })

  test('运行专员验证后 — 验证阶段包含 durationMs', async () => {
  ...
  test('运行专员获取 has-broken — 未验证时为 false（无 broken 链路）', () => {
    ctrl.resetAll()
    const broken = ctrl.getHasBroken()
    assert.equal(broken.hasBroken, false)
  })
...
describe(`${ROLES.GroupBuilding} cross-module 角色测试`, () => {
  ...
  test('团建专员查看跨模块链路 — 总数为 4', () => {
    const result = ctrl.getChainStatus()
    assert.equal(result.total, 4)
  })

  test('团建专员 — 所有链路模块数均为正整数', () => {
    const result = ctrl.getChainStatus()
    for (const chain of result.chains) {
  ...
  test('团建专员获取验证摘要 — 摘要结构完整', () => {
    const summary = ctrl.getSummary()
    assert.ok(summary)
    assert.equal(typeof summary.total, 'number')
  })

  test('团建专员查看 multi-client-consistency — 含 miniapp 和 api', () => {
    const result = ctrl.getChainStatus()
    const chain = result.chains.find(
      (c: { name: string; modules: string[]; status: string }) => c.name === 'multi-client-consistency'
    )
  ...
  test('团建专员仅读权限 — 存在 validate 方法但应有 guard 拦截', () => {
...
describe(`${ROLES.Marketing} cross-module 角色测试`, () => {
  ...
  test('营销专员可获取链路运行信息 — 各市场一致性检查', () => {
    const result = ctrl.getChainStatus()
    assert.equal(result.total, 4)
    assert.equal(result.runtime, 'cross-module-e2e')
  })

  test('营销专员获取摘要 — 用于跨市场推广一致性评估', () => {
    const summary = ctrl.getSummary()
    assert.ok(summary)
  })

  test('营销专员 — admin-to-consumer 链路 tenant 为起点', () => {
    const result = ctrl.getChainStatus()
    const chain = result.chains.find(
      (c: { name: string; modules: string[]; status: string }) => c.name === 'admin-to-consumer'
    )
  ...
  test('营销专员使用跨市场上下文 — us-default 不影响链路结构', () => {
    const result = ctrl.getChainStatus()
    // 市场代码不影响链路结构查询
    assert.equal(result.total, 4)
  })

  test('营销专员检查验证状态 — all-verified 接口可访问', () => {
    const verified = ctrl.getAllVerified()
    assert.ok('allVerified' in verified)
...
describe('cross-module 跨角色对比测试', () => {
  test('所有 8 角色均可获取 chain-status — 只读端点无角色限制', () => {
  ...
  test('验证后 has-broken 为 false, all-verified 为 true', async () => {
    const ctrl = createCtrl()
    await ctrl.validate({ chainNames: undefined, ...defaultCtx })
