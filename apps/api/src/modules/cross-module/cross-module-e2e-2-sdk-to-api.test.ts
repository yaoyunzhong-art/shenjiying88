...
test('E2E链#2 正例: domain枚举 → market service → bootstrap合同 全链路类型一致', () => {
...
test('E2E链#2 正例: domain枚举 → workbench service → bootstrap合同 角色-客户端映射正确', () => {
  const f = new FoundationService();
...
test('E2E链#2 反例: workbench 不应将导购角色分配给 PC-only 客户端', () => {
  const f = new FoundationService();
  const w = new WorkbenchService(f);
...
test('E2E链#2 边界: domain 枚举映射到 storefront surface 应完整', () => {
...
test('E2E链#2 边界: 所有 user role 应有对应 workbench', () => {
  const f = new FoundationService();
  const w = new WorkbenchService(f);
  const ctx = { tenantId: 'demo', marketCode: 'cn-mainland' };
  const bootstrap = w.getBootstrap(ctx);
...
test('E2E链#2 边界: SDK contract 跨模块传递时 market profiles 不应有重复 marketCode', () => {
  const f = new FoundationService();
  const m = new MarketService(f);
