...
beforeAll(() => { resetNoticeServiceTestState() })
afterAll(() => { resetNoticeServiceTestState() })
...
describe('👔店长 notice 扩展测试', () => {
  ...
  it('店长创建门店级公告并查看详情', () => {
    const notice = svc.controller.create(defaultTenantCtx, {
      title: '明日设备检修通知',
    ...
    expect(notice.title).toBe('明日设备检修通知')
    expect(notice.status).toBe('DRAFT')
    expect(notice.scope).toBe('STORE')
  })

  it('店长发布公告后状态变为 PUBLISHED', () => {
    const created = svc.controller.create(defaultTenantCtx, {
      title: '开业通知', content: '今日正常营业',
      scope: NoticeScope.Store, priority: NoticePriority.Normal,
      authorId: 'mgr-002', authorName: '李店长',
    })
    const published = svc.controller.publish(created.code)
    expect(published.status).toBe('PUBLISHED')
    expect(published.publishedAt).toBeDefined()
  })

  it('店长查询公告列表包含已创建的公告', () => {
    svc.controller.create(defaultTenantCtx, {
      title: '盘点通知', content: '今晚盘点',
      scope: NoticeScope.Store, priority: NoticePriority.Normal,
      authorId: 'mgr-003', authorName: '店长张',
    })
    const list = svc.controller.list(defaultTenantCtx, {})
...
describe('🛒前台 notice 扩展测试', () => {
  ...
  it('前台查看已发布公告列表', () => {
    ...
    svc.controller.publish(created.code)
    const published = svc.controller.listPublished({})
  ...
  it('前台标记公告为已读', () => {
    ...
    svc.controller.publish(created.code)
    const read = svc.controller.markRead(created.code, {
      userId: 'recept-001',
      userName: '前台小赵',
    })
  ...
  it('前台重复标记已读不增加计数', () => {
    ...
    svc.controller.publish(created.code)
    svc.controller.markRead(created.code, { userId: 'recept-002', userName: '前台' })
    svc.controller.markRead(created.code, { userId: 'recept-002', userName: '前台' })
    const detail = svc.controller.getById(created.code, 'recept-002')!
...
describe('👥HR notice 扩展测试', () => {
  ...
  it('HR 创建员工福利公告（品牌级）', () => {
    ...
    expect(notice.scope).toBe('BRAND')
    expect(notice.tags).toContain('福利')
  })
  ...
  it('HR 更新公告内容', () => {
    ...
    const updated = svc.controller.update(created.code, {
      title: '【更新】培训通知',
      content: '更新后的培训安排',
    })
  ...
  it('HR 删除草稿公告', () => {
    ...
    const result = svc.controller.delete(created.code)
    expect(result.id).toBe(created.code)
    // 已删除不可再获取
    const detail = svc.controller.getById(created.code)
...
describe('🔧安监 notice 扩展测试', () => {
  ...
  it('安监创建紧急安全公告（URGENT 级别）', () => {
    ...
    expect(notice.priority).toBe('URGENT')
  })
  ...
  it('安监创建带定时发布的公告', () => {
    ...
    expect(notice.scheduledAt).toBe(futureTime)
  })
  ...
  it('安监按优先级筛选公告', () => {
    ...
    const urgentList = svc.controller.list(defaultTenantCtx, { priority: 'URGENT' })
...
describe('🎮导玩员 notice 扩展测试', () => {
  ...
  it('导玩员创建游戏区设备公告', () => {
    ...
    expect(notice.authorName).toBe('导玩员小张')
  })

  it('导玩员创建置顶公告', () => {
    const notice = svc.controller.create(defaultTenantCtx, {
      title: '【置顶】新游戏上架',
      content: '新到4台格斗游戏机，欢迎体验',
      scope: NoticeScope.Store,
      priority: NoticePriority.Normal,
      authorId: 'guide-002',
      authorName: '导玩员小李',
    ...
    expect(notice.stickyOrder).toBe(10)
  })
  ...
  it('导玩员发布公告后查询已发布列表', () => {
    ...
    svc.controller.publish(created.code)
    const publishedList = svc.controller.listPublished({ scope: NoticeScope.Store })
...
describe('🎯运行专员 notice 扩展测试', () => {
  ...
  it('运行专员创建系统级公告', () => {
    ...
    expect(notice.scope).toBe('SYSTEM')
  })
  ...
  it('运行专员归档已发布公告', () => {
    ...
    svc.controller.publish(created.code)
    const archived = svc.controller.archive(created.code)
  ...
  it('运行专员按作者筛选公告', () => {
    ...
    const list = svc.controller.list(defaultTenantCtx, { authorId: 'ops-003' })
...
describe('🤝团建 notice 扩展测试', () => {
  ...
  it('团建创建活动公告带封面图', () => {
    ...
    expect(notice.coverUrl).toBeDefined()
    expect(notice.summary).toBe('季度团建报名通知')
  })
  ...
  it('团建创建带过期时间的公告', () => {
    ...
    expect(notice.expireAt).toBe(futureExpire)
  })
  ...
  it('团建通过关键字搜索自己的公告', () => {
    ...
    const list = svc.controller.list(defaultTenantCtx, { keyword: '海边' })
    expect(list.total).toBeGreaterThanOrEqual(1)
  })
})
...
describe('📢营销 notice 扩展测试', () => {
  ...
  it('营销创建促销活动公告', () => {
    ...
    expect(notice.tags).toContain('促销')
  })
  ...
  it('营销发布公告后按 scope 过滤', () => {
    ...
    const brandList = svc.controller.list(defaultTenantCtx, { scope: NoticeScope.Brand })
  ...
  it('营销查看未读公告已读状态为 false', () => {
    ...
    const detail = svc.controller.getById(created.code, 'mkt-viewer-001')!
    expect(detail).toBeDefined()
