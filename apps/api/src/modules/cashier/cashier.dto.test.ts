...
describe('CashierOrderItemDto', () => {
  ...
  it('quantity 为 0 时的订单项（边界：白送商品/赠品）', () => {
    const item = Object.assign(new CashierOrderItemDto(), {
      skuId: 'SKU-FREE',
      quantity: 0,
      price: 0,
  ...
  it('price 为负数时 DTO 仍然接受（由 service 层做业务校验）', () => {
    const item = Object.assign(new CashierOrderItemDto(), {
      skuId: 'SKU-REFUND',
      quantity: 1,
      price: -99,
...
describe('CreateCashierOrderDto', () => {
  ...
  it('创单 DTO 带盲盒参数 blindboxPlanId + blindboxQuantity', () => {
    const dto = Object.assign(new CreateCashierOrderDto(), {
      memberId: 'member-4',
      items: [
  ...
  it('创单 DTO items 为空数组时的边界（service 层应拒单）', () => {
    const dto = Object.assign(new CreateCashierOrderDto(), {
      memberId: 'member-5',
      items: []
    })
...
describe('CreateCashierPaymentDto', () => {
  ...
  it('创建支付 DTO 指定 amount 覆盖 order total', () => {
    const dto = Object.assign(new CreateCashierPaymentDto(), {
  ...
  it('创建支付 DTO 仅 channel 不填 amount（由 service 取 order total）', () => {
    const dto = Object.assign(new CreateCashierPaymentDto(), {
      channel: 'card'
    })
  ...
  it('创建支付 DTO channel 为自定义内部支付方式', () => {
    const dto = Object.assign(new CreateCashierPaymentDto(), {
...
describe('CashierPaymentCallbackDto', () => {
  ...
  it('支付回调 DTO 必填字段校验：无 tenantId 也能创建 DTO（由 service 做校验）', () => {
    const dto = Object.assign(new CashierPaymentCallbackDto(), {
      standardizedEventName: 'cashier.payment-succeeded' as const,
      aggregateId: 'agg-3',
      orderId: 'order-3',
  ...
  it('支付回调 DTO 事件名只能为 payment-succeeded 或 payment-failed', () => {
