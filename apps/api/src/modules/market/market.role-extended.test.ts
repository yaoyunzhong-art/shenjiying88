...
function mockMarketService(overrides = {}) {
  return {
    ...
    getMergedProfile: (ctx: { marketCode: string }) => (ctx.marketCode === 'us-default' ? usProfile : cnProfile),
    getOverrides: () => [],
    ...overrides,
  }
}

function createMarketController(overrides = {}) {
  const svc = mockMarketService(overrides)
...
describe(`${ROLES.TenantAdmin} market 扩展角色测试`, () => {
  test('店长查看 bootstrap 获得所有市场列表与 foundation 元数据', () => {
  ...
  test('店长跨市场查看 scoped market——中国市场含完整 locale/currency/tax/social', () => {
    const { controller } = createMarketController()
  ...
  test('店长跨市场查看 scoped market——美国市场含英文 locale 与美元', () => {
    const { controller } = createMarketController()
  ...
  test('店长查看 US portal 视图——仅返回 portal 相关字段不含完整 profile', () => {
    const { controller } = createMarketController()
...
describe(`${ROLES.Reception} market 扩展角色测试`, () => {
  test('前台获取当前门店的 portal 市场配置（含税率与邮件配置）', () => {
    const { controller } = createMarketController()
  ...
  test('前台获取跨区域市场 portal——中美时区不同', () => {
    ...
    assert.equal(cnPortal.timezone, 'Asia/Shanghai')
    assert.equal(usPortal.timezone, 'America/New_York')
    assert.notEqual(cnPortal.timezone, usPortal.timezone)
  })

  test('前台访问不存在的 scopeCode 仍然返回有效市场配置（回退逻辑）', () => {
    const { controller } = createMarketController()
...
describe(`${ROLES.HR} market 扩展角色测试`, () => {
  ...
  test('HR 查看 bootstrap 确认所有市场都有完整的税务配置', () => {
    const { controller } = createMarketController()
    const result = controller.getBootstrap()
...
describe(`${ROLES.Safety} market 扩展角色测试`, () => {
  ...
  test('安监验证 bootstrap 中所有市场 network 配置完整', () => {
    const { controller } = createMarketController()
    const result = controller.getBootstrap()
...
describe(`${ROLES.Guide} market 扩展角色测试`, () => {
  test('导玩员获取中国大陆门店的 portal 视图含社交平台信息', () => {
    const { controller } = createMarketController()
    const result = controller.getScopedPortalMarket('store', 's-cn', cnTenantCtx)
  ...
  test('导玩员获取美国门店 portal 视图时社交平台不同', () => {
    const { controller } = createMarketController()
    const result = controller.getScopedPortalMarket('store', 's-us', usTenantCtx)
  ...
  test('导玩员调用 scopedMarket 查看门店 override——含自定义 email fromName', () => {
    const overrides = [
...
describe(`${ROLES.Ops} market 扩展角色测试`, () => {
  test('运行专员验证 bootstrap 返回的 foundation 合同完整', () => {
    const { controller } = createMarketController()
    const result = controller.getBootstrap()
  ...
  test('运行专员验证 override 链：tenant → brand → store 逐层合并', () => {
    const overrides = [
  ...
  test('运行专员验证无 override 时返回 base profile', () => {
...
describe(`${ROLES.Teambuilding} market 扩展角色测试`, () => {
  ...
  test('团建负责人获取中美两个市场的 portal 视图对比语言', () => {
    const { controller } = createMarketController()
    const cnPortal = controller.getScopedPortalMarket('store', 's-cn', cnTenantCtx)
    const usPortal = controller.getScopedPortalMarket('store', 's-us', usTenantCtx)

    assert.equal(cnPortal.locale, 'zh-CN')
    assert.equal(usPortal.locale, 'en-US')
  })
})
...
describe(`${ROLES.Marketing} market 扩展角色测试`, () => {
  ...
  test('营销查看 bootstrap 确认两个市场都有完整的 email 配置用于邮件营销', () => {
    const { controller } = createMarketController()
    const result = controller.getBootstrap()
  ...
  test('营销验证 portal 视图含社交与税务信息但不含完整 profile 对象', () => {
    const { controller } = createMarketController()
