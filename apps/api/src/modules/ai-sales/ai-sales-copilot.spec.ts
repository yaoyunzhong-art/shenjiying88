...
describe('AiSalesService (Complete)', () => {
  ...
  it('不同客户应返回不同推荐', () => {
    const recs1 = service.getRecommendations('cust-001')
    const recs2 = service.getRecommendations('cust-002')
    expect(recs1).not.toEqual(recs2)
  })

  it('handlePriceObjection 应返回说服话术', () => {
    const response = service.handlePriceObjection('prod-001', '价格太贵')
    expect(response).toBeTruthy()
    expect(typeof response).toBe('string')
  })

  it('handleQualityObjection 应返回质量话术', () => {
    const response = service.handlePriceObjection('prod-001', '质量如何保证')
    expect(response).toBeTruthy()
  })
  ...
  it('getConversationSimulation 应返回模拟对话', () => {
    const sim = service.getConversationSimulation?.('cust-001', 'prod-001', ['价格多少'])
    if (sim) {
