...
describe('MemberDormancyCron', () => {
  ...
  describe('hourlyScan', () => {
    it('正例: 空数据返回 scannedCount=0', async () => {
      const ds: any = dormancy
      ds.memberService = { listProfiles: () => [], getProfile: () => undefined }
    ...
    it('正例: 有会员进入 DORMANT 阶段', async function(t) {
      const inactiveSince = '2026-03-28T06:00:00.000Z'  // 91 days before now
      const ds: any = dormancy
      ds.memberService = {
        listProfiles: () => [makeMember({ memberId: 'm1', lastActiveAt: inactiveSince })],
    ...
    it('正例: 扫描结果含 configSnapshot', async function(t) {
      const inactiveSince = '2026-03-28T06:00:00.000Z'  // 91 days before now
      const ds: any = dormancy
      ds.memberService = {
        listProfiles: () => [makeMember({ memberId: 'm2', lastActiveAt: inactiveSince })],
        getProfile: () => undefined
      }
      const result = await cron.hourlyScan()
