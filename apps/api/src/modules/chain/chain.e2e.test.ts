...
it('e2e: 积分清算合约创建→审批→执行→查询全流程', async () => {
  ...
  try {
    ...
    const contractId = createRes.body.data.data.contractId
    assert.ok(contractId)
    assert.equal(createRes.body.data.data.status, 'Created')
    assert.equal(createRes.body.data.data.totalAmount, 800)

    // 2. 审批合约
    const approveRes = await request(app.getHttpServer())
      .post(`/chain/settlements/${contractId}/approve`)
    assert.equal(approveRes.statusCode, 201)
    assert.equal(approveRes.body.data.data.status, 'Approved')
    assert.ok(approveRes.body.data.data.approvedAt)
    ...
    assert.equal(execRes.body.data.data.status, 'Completed')
    assert.ok(execRes.body.data.data.executedAt)

    // 4. 查询合约状态
    const getRes = await request(app.getHttpServer())
      .get(`/chain/settlements/${contractId}`)
    assert.equal(getRes.statusCode, 200)
    assert.equal(getRes.body.data.data.status, 'Completed')
    assert.ok(getRes.body.data.data.participants.every((p: Record<string, unknown>) => p.transferred === true))
  } finally {
    await app.close()
  }
})
...
it('e2e: 分账合约创建→分配→参与者份额查询', async () => {
  ...
  try {
    ...
    const contractId = createRes.body.data.data.contractId
    assert.ok(contractId)
    assert.equal(createRes.body.data.data.totalRevenue, 10000)
    assert.equal(createRes.body.data.data.status, 'Created')
    ...
    assert.equal(distRes.body.data.data.status, 'Completed')
    ...
    assert.equal(shareRes.body.data.data.expected, 5000)
    assert.equal(shareRes.body.data.data.distributed, true)

    // 4. 查询分账历史
    const historyRes = await request(app.getHttpServer())
      .get(`/chain/revenue-shares/${contractId}/history`)
    assert.equal(historyRes.statusCode, 200)
    assert.ok(historyRes.body.data.data.length >= 3)
  } finally {
    await app.close()
  }
})
...
it('e2e: 合约执行器部署→执行→结果查询', async () => {
  ...
  try {
    ...
    const settlementId = createRes.body.data.data.contractId
    ...
    const deployedId = deployRes.body.data.data.deployedContractId
    ...
    assert.equal(execRes.statusCode, 201)
    assert.equal(execRes.body.data.data.success, true)
    ...
    assert.ok(resultRes.body.data.data.executedAt)
  } finally {
    await app.close()
  }
})
...
it('e2e: 链上智能合约部署→方法调用→Gas估算', async () => {
  ...
  try {
    ...
    assert.equal(deployRes.statusCode, 201)
    const contractId = deployRes.body.data.data.contractId
    assert.ok(contractId)
    assert.ok(deployRes.body.data.data.address)
    ...
    assert.ok(gasRes.body.data.data.gas > 0)

    // 4. 列出合约
    const listRes = await request(app.getHttpServer())
      .get('/chain/smart-contracts')
    assert.equal(listRes.statusCode, 200)
    assert.ok(listRes.body.data.data.length >= 1)
  } finally {
    await app.close()
  }
})
...
it('e2e: 积分清算合约取消流程', async () => {
  ...
  try {
    ...
    assert.equal(cancelRes.body.data.data.status, 'Cancelled')
    assert.ok(cancelRes.body.data.data.cancelledAt)
  } finally {
    await app.close()
  }
})
...
it('e2e: 智能合约验证与事件查询', async () => {
  ...
  try {
    ...
    assert.equal(verifyRes.body.data.data.verified, true)

    // 查询事件
    const eventsRes = await request(app.getHttpServer())
      .get(`/chain/smart-contracts/${contractId}/events`)
    assert.equal(eventsRes.statusCode, 200)
    assert.ok(Array.isArray(eventsRes.body.data.data))
  } finally {
    await app.close()
  }
})
