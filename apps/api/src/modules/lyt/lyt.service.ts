...
import type {
  ...
  LytWebhookDrillContract
} from './lyt.contract'
import {
  evaluateLytFixtureValidation,
  getMissingFixtureFields,
  getMissingFixtureHeaders,
  getMissingFixtureQueryParams,
  getLytFixtureByKey,
  getLytFixtureCatalog,
  type LytFixtureCapability,
  type LytFixtureTransport
} from './lyt-fixture.catalog'
import { computeDeviceHealthSummary } from './lyt.entity'
import type { RequestTenantContext } from '../tenant/tenant.types'

@Injectable()
export class LytService {
  private readonly governedCapabilities = ['member', 'payment', 'order', 'device', 'gate', 'coin', 'inventory', 'shelf']

  constructor(
    private readonly adapterRegistry: LytAdapterRegistry,
    private readonly foundationService: FoundationService,
    private readonly connectionManager: LytConnectionManager,
    ...
    @Optional() private readonly governanceQueryService?: LytGovernanceQueryService,
  ...
  private async emitAudit(input: {
    ...
    const riskLevel =
      input.riskLevel ?? this.resolveWebhookRuntimeRiskLevel({ capability: input.standardizedEvent.capability })
  ...
  private resolveWebhookRuntimeRiskLevel(standardizedEvent: {
    capability: 'member' | 'payment' | 'order' | 'device' | 'gate' | 'coin' | 'coupon' | 'unknown'
  ...
  private buildWebhookRuntimePayloadSummary(input: {
  ...
  private async attachWebhookRuntimeReceipt(input: {
    acceptedStatus: string
    aggregateId: string
    ...
    return this.runtimeGovernanceService.recordCallback(runtimeReceipt.receiptCode, {
      ...
      summary:
        input.acceptedStatus === 'duplicate'
  ...
  private async syncMemberSnapshotFromStandardizedEvent(standardizedEvent: {
    standardizedEventName: string
    ...
    const externalMemberId =
      typeof standardizedEvent.payload.externalMemberId === 'string'
    ...
    try {
      const synced = await this.memberService.syncLytMemberSnapshot({
        ...
        externalMemberId,
        memberCode:
          typeof standardizedEvent.payload.memberCode === 'string'
            ? standardizedEvent.payload.memberCode
            : undefined,
        mobile:
          typeof standardizedEvent.payload.mobile === 'string'
            ? standardizedEvent.payload.mobile
            : undefined,
        nickname:
          typeof standardizedEvent.payload.nickname === 'string'
            ? standardizedEvent.payload.nickname
            : undefined,
        levelCode:
          typeof standardizedEvent.payload.levelCode === 'string'
            ? standardizedEvent.payload.levelCode
            : undefined,
        points:
          typeof standardizedEvent.payload.points === 'number'
            ? standardizedEvent.payload.points
            : undefined,
        growthValue:
          typeof standardizedEvent.payload.growthValue === 'number'
            ? standardizedEvent.payload.growthValue
            : undefined,
        status:
          typeof standardizedEvent.payload.status === 'string'
            ? standardizedEvent.payload.status
            : undefined,
        updatedAt:
          typeof standardizedEvent.payload.updatedAt === 'string'
  ...
  private async syncOrderSnapshotFromStandardizedEvent(standardizedEvent: {
    standardizedEventName: string
    payload: Record<string, unknown>
    tenantId?: string
    brandId?: string
    ...
    const externalOrderId =
      typeof standardizedEvent.payload.externalOrderId === 'string'
    ...
    try {
      const snapshot = await this.transactionsService.syncLytOrderSnapshot({
        ...
        externalOrderId,
        orderNo:
          typeof standardizedEvent.payload.orderNo === 'string'
            ? standardizedEvent.payload.orderNo
            : undefined,
        memberId:
          typeof standardizedEvent.payload.memberId === 'string'
            ? standardizedEvent.payload.memberId
            : undefined,
        couponCode:
          typeof standardizedEvent.payload.couponCode === 'string'
            ? standardizedEvent.payload.couponCode
            : undefined,
        blindboxPlanId:
          typeof standardizedEvent.payload.blindboxPlanId === 'string'
            ? standardizedEvent.payload.blindboxPlanId
            : undefined,
        blindboxQuantity:
          typeof standardizedEvent.payload.blindboxQuantity === 'number'
            ? standardizedEvent.payload.blindboxQuantity
            : undefined,
        amount:
          typeof standardizedEvent.payload.amount === 'number'
            ? standardizedEvent.payload.amount
            : undefined,
        discountAmount:
          typeof standardizedEvent.payload.discountAmount === 'number'
            ? standardizedEvent.payload.discountAmount
            : undefined,
        payableAmount:
          typeof standardizedEvent.payload.payableAmount === 'number'
            ? standardizedEvent.payload.payableAmount
            : undefined,
        currency:
          typeof standardizedEvent.payload.currency === 'string'
        ...
        status:
          typeof standardizedEvent.payload.status === 'string'
            ...
            : undefined,
        paidAt:
          typeof standardizedEvent.payload.paidAt === 'string'
  ...
  private async triggerSnapshotConsumersFromStandardizedEvent(input: {
    standardizedEventName: string
    tenantContext: RequestTenantContext
    memberSnapshotSync?: Record<string, unknown> | null
    orderSnapshotSync?: Record<string, unknown> | null
    paymentSnapshotSync?: Record<string, unknown> | null
    payload: Record<string, unknown>
  }) {
    if (input.standardizedEventName === 'member.profile-synced') {
      ...
      const memberId =
        typeof input.memberSnapshotSync?.memberProfileId === 'string'
      ...
      const operationsTasks =
        operationsProfile.recommendedActions.length > 0
      ...
      return {
        ...
        memberOperationsSync: {
          ...
          executedRuntimeReceiptCodes:
            operationsTasks?.executedReceipts
    ...
    if (input.standardizedEventName === 'cashier.order-updated') {
      ...
      const externalOrderId =
        typeof input.orderSnapshotSync?.externalOrderId === 'string'
    ...
    const externalPaymentId =
      typeof input.paymentSnapshotSync?.externalPaymentId === 'string'
    ...
    if (!orderSnapshot) {
      orderSnapshot = await this.transactionsService.syncLytOrderSnapshot({
        ...
        externalOrderId,
        orderNo:
          typeof input.payload.orderNo === 'string'
        ...
        memberId:
          typeof input.payload.memberId === 'string'
            ...
            : undefined,
        couponCode:
          typeof input.payload.couponCode === 'string'
            ? input.payload.couponCode
            : undefined,
        blindboxPlanId:
          typeof input.payload.blindboxPlanId === 'string'
            ? input.payload.blindboxPlanId
            : undefined,
        blindboxQuantity:
          typeof input.payload.blindboxQuantity === 'number'
        ...
        discountAmount:
          typeof input.payload.discountAmount === 'number'
            ? input.payload.discountAmount
            : 0,
        payableAmount:
          typeof input.payload.payableAmount === 'number'
        ...
        status:
          typeof input.payload.status === 'string'
            ...
            : input.standardizedEventName === 'cashier.payment-succeeded'
              ...
              : 'PAYMENT_FAILED',
        paidAt:
          typeof input.payload.paidAt === 'string'
            ? input.payload.paidAt
            : paymentSnapshot.paidAt,
        updatedAt:
          typeof input.payload.updatedAt === 'string'
    ...
    return {
      ...
      memberOperationsSync: memberOperations
        ? {
            ...
            executedReceiptIds:
              memberOperationsTasks?.executedReceipts.map((receipt) => receipt.executionId) ?? [],
            executedRuntimeReceiptCodes:
              memberOperationsTasks?.executedReceipts
                .map((receipt) => receipt.runtimeReceiptCode)
                .filter((code): code is string => Boolean(code)) ?? []
          }
        : null
  ...
  private async syncPaymentSnapshotFromStandardizedEvent(standardizedEvent: {
    standardizedEventName: string
    payload: Record<string, unknown>
    tenantId?: string
    brandId?: string
    ...
    const externalPaymentId =
      typeof standardizedEvent.payload.externalPaymentId === 'string'
      ...
      const snapshot = await this.transactionsService.syncLytPaymentSnapshot({
        ...
        currency:
          typeof standardizedEvent.payload.currency === 'string'
            ...
            : undefined,
        transactionNo:
          typeof standardizedEvent.payload.transactionNo === 'string'
  ...
  async getAdapterSelection(
    storeId: string,
    tenantContext?: RequestTenantContext
  ): Promise<LytAdapterSelectionContract> {
  ...
  getFixtures(filters?: {
    ...
    const catalog = getLytFixtureCatalog({
      transport: filters?.transport as LytFixtureTransport | undefined,
      capability: filters?.capability as LytFixtureCapability | undefined
    })

    return catalog.map((item) => {
      const validation = evaluateLytFixtureValidation(item)

      return toLytFixtureCatalogItemContract({
        key: item.key,
        title: item.title,
  ...
  getFixtureSummary(filters?: {
    ...
    const missingFieldBreakdown = fixtures.reduce<Record<string, number>>((acc, fixture) => {
      for (const field of fixture.missingSampleFields) {
        acc[field] = (acc[field] ?? 0) + 1
      }
      return acc
    }, {})
    const missingChecklistBreakdown = fixtures.reduce<Record<string, number>>((acc, fixture) => {
      for (const item of fixture.missingChecklistItems) {
        acc[item] = (acc[item] ?? 0) + 1
      }
      return acc
    }, {})
    const recommendedChecklistBreakdown = fixtures.reduce<Record<string, number>>((acc, fixture) => {
      for (const item of [
    ...
    return {
      ...
      highRiskBlockedFixtures: fixtures.filter(
        (fixture) => fixture.riskLevel === 'high' && fixture.validationStatus === 'needs-sample-completion'
      ).length,
      blockedFixtureKeys,
      transportBreakdown: fixtures.reduce<Record<'api' | 'webhook', number>>(
        (acc, fixture) => {
      ...
      capabilityBreakdown: fixtures.reduce<Partial<Record<'member' | 'order' | 'payment' | 'gate' | 'device', number>>>(
        (acc, fixture) => {
  ...
  getFixture(key: string) {
    const fixture = getLytFixtureByKey(key)
    if (!fixture) {
      throw new NotFoundException(`LYT fixture not found: ${key}`)
    }

    const validation = evaluateLytFixtureValidation(fixture)
  ...
  compareFixtureInput(key: string, input: LytFixtureCompareDto) {
    ...
    const payloadMissingRequired = getMissingFixtureFields(fixture, payload)
    const headersMissingRequired = getMissingFixtureHeaders(fixture, headers)
    const queryMissingRequired = getMissingFixtureQueryParams(fixture, query)
    ...
    return {
      ...
      readiness:
        payloadMissingRequired.length || headersMissingRequired.length || queryMissingRequired.length
          ? 'missing-required'
          : 'ready',
      ...
      recommendedNextActions: this.buildCompareRecommendations({
        ...
        riskyPayloadExtras: this.classifyExtraObservedKeys(
          payload,
          [...fixture.requiredRawFields, ...fixture.recommendedRawFields],
          'payload'
        ).riskyExtraObserved,
        riskyHeaderExtras: this.classifyExtraObservedKeys(
          headers,
          [...fixture.requiredHeaders, ...fixture.recommendedHeaders],
          'headers'
        ).riskyExtraObserved,
        riskyQueryExtras: this.classifyExtraObservedKeys(
          query,
          [...fixture.requiredQueryParams, ...fixture.recommendedQueryParams],
          'query'
        ).riskyExtraObserved
      })
    }
  ...
  previewFixtureImport(key: string, input: LytFixtureImportPreviewDto) {
    ...
    return {
      ...
      changedSections: [
        ...(this.getChangedKeys(fixture.samplePayload, nextSamplePayload).length > 0 ? (['payload'] as const) : []),
        ...(this.getChangedKeys(fixture.sampleHeaders, nextSampleHeaders).length > 0 ? (['headers'] as const) : []),
        ...(this.getChangedKeys(fixture.sampleQueryParams, nextSampleQueryParams).length > 0 ? (['query'] as const) : [])
      ],
      changedKeys: {
  ...
  planFixtureImport(key: string, input: LytFixtureImportPlanDto) {
    ...
    const payloadSection = this.buildImportPlanSection(
      fixture.samplePayload,
      preview.nextSamplePayload,
      compareBeforeImport.payload.safeExtraObserved,
      compareBeforeImport.payload.riskyExtraObserved,
      compareAfterImport.payload.missingRequired,
      compareAfterImport.payload.missingRecommended
    )
    const headersSection = this.buildImportPlanSection(
      fixture.sampleHeaders,
      preview.nextSampleHeaders,
      compareBeforeImport.headers.safeExtraObserved,
      compareBeforeImport.headers.riskyExtraObserved,
      compareAfterImport.headers.missingRequired,
      compareAfterImport.headers.missingRecommended
    )
    const querySection = this.buildImportPlanSection(
      fixture.sampleQueryParams,
      preview.nextSampleQueryParams,
      compareBeforeImport.query.safeExtraObserved,
      compareBeforeImport.query.riskyExtraObserved,
      compareAfterImport.query.missingRequired,
      compareAfterImport.query.missingRecommended
    )
    const riskyExtraCount =
      payloadSection.riskyExtraCandidates.length +
      headersSection.riskyExtraCandidates.length +
      querySection.riskyExtraCandidates.length
    const importDecision =
      compareAfterImport.readiness === 'missing-required'
        ? 'blocked-by-required'
        : riskyExtraCount > 0
          ? 'needs-review'
          : 'ready-to-promote'
  ...
  private buildFixtureSummaryRecommendations(
    ...
    const highRiskBlocked = fixtures.filter(
      (fixture) => fixture.riskLevel === 'high' && fixture.validationStatus === 'needs-sample-completion'
    )
  ...
  private getMissingOptionalKeys(keys: string[], input: Record<string, unknown>) {
  ...
  private isSafeExtraObservedKey(key: string, scope: 'payload' | 'headers' | 'query') {
  ...
  private getRecommendedPromotions(sections: {
    payload: {
      add: string[]
      update: string[]
      safeExtraCandidates: string[]
    ...
    for (const scope of ['payload', 'headers', 'query'] as const) {
  ...
  private buildImportPlanRecommendations(input: {
    ...
    if (
      input.payloadSection.riskyExtraCandidates.length ||
    ...
    if (
      input.headersSection.add.includes('signature') ||
    ...
    if (
      input.compareAfterImport.payload.missingRecommended.length ||
    ...
    if (
      input.importDecision === 'ready-to-promote' &&
  ...
  private buildCompareRecommendations(input: {
    payloadMissingRequired: string[]
    headersMissingRequired: string[]
    queryMissingRequired: string[]
    payloadMissingRecommended: string[]
    headersMissingRecommended: string[]
    queryMissingRecommended: string[]
    riskyPayloadExtras: string[]
    riskyHeaderExtras: string[]
    riskyQueryExtras: string[]
  }) {
    const actions: string[] = []
  ...
  async replayWebhookFixture(input: LytWebhookFixtureReplayDto) {
    const fixture = getLytFixtureByKey(input.fixtureKey)
    if (!fixture) {
      throw new NotFoundException(`LYT fixture not found: ${input.fixtureKey}`)
    }

    if (fixture.transport !== 'webhook') {
      throw new BadRequestException(`LYT fixture is not webhook-capable: ${input.fixtureKey}`)
    }
    ...
    const missingChecklistItems = [
      ...getMissingFixtureFields(fixture, payload).map((field) => `payload:${field}`),
      ...getMissingFixtureHeaders(fixture, headers).map((field) => `headers:${field}`),
      ...getMissingFixtureQueryParams(fixture, query).map((field) => `query:${field}`)
    ]
    if (input.strictValidation && missingChecklistItems.length > 0) {
      throw new BadRequestException(
        `LYT fixture input missing required checklist items: ${missingChecklistItems.join(', ')}`
      )
    }
    ...
    return this.acceptWebhook({
      ...
      const standardizedEvent = result.standardizedEvent as
        | {
            aggregateId?: string
            tenantId?: string
            brandId?: string
      ...
      if (standardizedEvent?.aggregateId) {
        ...
        const capability =
          (standardizedEvent.capability as
            | 'member'
            | 'payment'
            | 'order'
            | 'device'
        ...
        await this.emitAudit({
          ...
          details: {
            ...
            acceptedStatus: (result as { status?: string }).status ?? 'accepted'
          }
        })
      }
      return result
  ...
  async drillWebhook(input: LytWebhookDrillDto): Promise<LytWebhookDrillContract> {
    const fixture = input.fixtureKey ? getLytFixtureByKey(input.fixtureKey) : null
    ...
    const archiveRecord = toLytWebhookArchiveRecordContract({
      source: 'lyt-drill',
      standardizedEvent,
    ...
    const standardizedPublication = await this.integrationOrchestrationService.publishEvent(
      standardizedEvent.standardizedEventName,
      standardizedEvent.payload,
      {
        source: 'lyt-drill',
        aggregateId: standardizedEvent.aggregateId,
  ...
  async acceptWebhook(input: LytWebhookIngestDto): Promise<Record<string, unknown>> {
    ...
    const archiveRecord = toLytWebhookArchiveRecordContract({
      ...
      signatureVerified:
        typeof accepted.signatureVerified === 'boolean' ? accepted.signatureVerified : false
    })
   ...
   */
  getDeviceHealthSummary(devices: import('./lyt.entity').LytDevice[], thresholdMinutes?: number) {
    return computeDeviceHealthSummary(devices, thresholdMinutes)
  }
}
