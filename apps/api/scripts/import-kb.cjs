 ...
 * 使用 prisma $queryRaw 避免 pg 依赖问题
 */
const { createHash } = require('crypto');
const { readFileSync, readdirSync } = require('fs');
const { join, relative, basename } = require('path');

// 直接在 apps/api 上下文中运行 prisma
...
async function loadCategories() {
  const rows = await prisma.$queryRawUnsafe('SELECT id, path_prefix FROM kb_category ORDER BY sort_order');
  const map = {};
  for (const r of rows) {
    map[r.path_prefix] = r.id;
  }
  return map;
}
...
async function main() {
  ...
  const existing = await prisma.$queryRawUnsafe(
    `SELECT file_path, content_hash FROM kb_document WHERE file_path = ANY($1::text[])`,
    relPaths
  );
  const existingMap = {};
