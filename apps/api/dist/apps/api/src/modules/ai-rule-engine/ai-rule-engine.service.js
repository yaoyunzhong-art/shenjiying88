...
exports.AiRuleEngineService = void 0;
const common_1 = require("@nestjs/common");
const domain_1 = require("@m5/domain");
let AiRuleEngineService = class AiRuleEngineService {
    memberLevelEngine = {
        id: 'member-level-v1',
    ...
    evaluateMemberLevel(input) {
        ...
        const conditionResults = conditions.map((cond) => {
            const matches = this.evaluateCondition(cond, input);
            if (matches)
                triggeredRules.push(cond.id);
            return matches;
        });
        // 根据匹配策略判定
        const isMatch = matchStrategy === 'ALL'
    ...
    evaluateCondition(condition, data) {
        ...
        switch (condition.operator) {
            case domain_1.PolicyConditionOperator.Eq:
                return fieldValue === expectedValue;
            case domain_1.PolicyConditionOperator.NotEq:
                return fieldValue !== expectedValue;
            case domain_1.PolicyConditionOperator.Gte:
                return (typeof fieldValue === 'number' &&
                    typeof expectedValue === 'number' &&
                    fieldValue >= expectedValue);
            case domain_1.PolicyConditionOperator.Lte:
                return (typeof fieldValue === 'number' &&
                    typeof expectedValue === 'number' &&
                    fieldValue <= expectedValue);
            case domain_1.PolicyConditionOperator.In:
                return (Array.isArray(expectedValue) &&
                    expectedValue.includes(fieldValue));
            case domain_1.PolicyConditionOperator.NotIn:
                return (Array.isArray(expectedValue) &&
                    !expectedValue.includes(fieldValue));
            case domain_1.PolicyConditionOperator.Exists:
     ...
     * 推断当前等级
     */
    inferCurrentLevel(input) {
        if (input.totalSpend >= 10000 && input.totalPoints >= 5000)
            return 'SVIP';
        if (input.totalSpend >= 5000 || input.totalPoints >= 2000)
            return 'VIP';
        return 'REGULAR';
    }
    /**
     ...
     */
    getEngineStatus() {
        return [this.memberLevelEngine, this.deviceAnomalyEngine, this.riskScoreEngine].map((engine) => ({
            engineId: engine.id,
     ...
     * 确定异常类型
     */
    determineAnomalyType(triggeredRules) {
        if (triggeredRules.includes('cond-cpu-high'))
            return 'CPU_SPIKE';
        if (triggeredRules.includes('cond-memory-high'))
            return 'MEMORY_LEAK';
        if (triggeredRules.includes('cond-disk-high'))
            return 'DISK_FULL';
        if (triggeredRules.includes('cond-network-slow'))
            return 'NETWORK_LATENCY';
        if (triggeredRules.includes('cond-error-high'))
    ...
    runSimulator(input) {
        ...
        const logs = input.verbose ? [] : undefined;
        // 应用条件覆盖
        const effectiveConditions = this.applyConditionOverrides(engine.conditions, input.conditionOverrides ?? [], logs);
        // 模拟条件评估
        const triggeredConditions = [];
        // 对于 device-anomaly / risk-score 类型，使用 metrics 子对象进行评估
        const rawData = input.data;
        const needsMetricsExtraction = (input.dataType === 'device-anomaly' || input.dataType === 'risk-score') &&
            rawData.metrics &&
            typeof rawData.metrics === 'object';
        const dataRecord = needsMetricsExtraction
            ? rawData.metrics
            : rawData;
        if (input.verbose)
            logs.push(`[SIM] Evaluating ${effectiveConditions.length} conditions`);
        const conditionResults = effectiveConditions.map((cond) => {
            ...
            if (matches) {
                ...
                if (input.verbose)
                    logs.push(`[SIM] Condition ${cond.id} MATCHED (field=${cond.field})`);
            }
            else {
                if (input.verbose)
                    logs.push(`[SIM] Condition ${cond.id} NOT matched`);
            }
            return matches;
        });
        // 匹配判定
        const matched = engine.matchStrategy === 'ALL'
            ? conditionResults.every(Boolean)
            : conditionResults.some(Boolean);
        if (input.verbose) {
            logs.push(`[SIM] Strategy=${engine.matchStrategy}, matched=${matched}, triggered=${triggeredConditions.length}`);
        }
        // 计算匹配分数
    ...
    runSimulatorBatch(input) {
        ...
        const results = [];
        const conditionCounter = new Map();
        ...
        for (let i = 0; i < rounds; i++) {
            ...
            const mutatedInput = {
                ...input,
                verbose: false,
                data: (runWithMutation ? this.mutateData(input.data, i) : input.data)
            };
     ...
     * 应用条件值覆盖
     */
    applyConditionOverrides(conditions, overrides, logs) {
        ...
        return conditions.map((cond) => {
            ...
            if (overrideValue !== undefined) {
                if (logs) {
                    logs.push(`[SIM] Override condition ${cond.id}: ${JSON.stringify(cond.value)} → ${JSON.stringify(overrideValue)}`);
                }
...
exports.AiRuleEngineService = AiRuleEngineService;
exports.AiRuleEngineService = AiRuleEngineService = __decorate([
    (0, common_1.Injectable)()
], AiRuleEngineService);
//# sourceMappingURL=ai-rule-engine.service.js.map