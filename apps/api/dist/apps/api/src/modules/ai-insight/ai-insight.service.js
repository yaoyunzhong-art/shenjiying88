...
exports.AiInsightService = void 0;
const common_1 = require("@nestjs/common");
/**
 * AI 经营洞察服务
 * KPI 看板 | 洞察报告 | 异常检测 | 趋势预测 | 仪表盘
 */
let AiInsightService = class AiInsightService {
    // ─── 模拟数据存储 ───
    /** KPI 指标池 */
    kpis = [];
    /** 异常记录池 */
     ...
     * 获取KPI列表，支持按分类筛选
     */
    getKPIs(tenantId, storeId, category) {
     ...
     */
    generateReport(tenantId, storeId, type, periodStart, periodEnd) {
        // 从 KPI 池中筛选关联的指标
        const relevantKPIs = this.kpis.filter((k) => k.tenantId === tenantId &&
        ...
        for (const kpi of relevantKPIs) {
            ...
            const changePercent = previousValue > 0 ? ((kpi.value - previousValue) / previousValue) * 100 : 0;
            trends.push({
                name: kpi.name,
                current: kpi.value,
            ...
            if ((kpi.trend === 'up' && kpi.value < threshold) ||
     ...
     * 基于 3-sigma 规则检测异常值
     */
    detectAnomalies(tenantId, storeId, metric) {
        // 按指标分组，计算均值与标准差
        const kpiPool = this.kpis.filter((k) => {
            if (k.tenantId !== tenantId)
                return false;
            if (storeId && k.storeId && k.storeId !== storeId)
                return false;
            if (metric && k.name !== metric)
                return false;
            return true;
        });
        // 按指标名称分组
            ...
            for (const value of values) {
                if (value > upperBound || value < lowerBound) {
                    ...
                    if (deviation > 50)
                        ...
                        severity = 'medium';
                    const existing = this.anomalies.find((a) => a.tenantId === tenantId &&
                        a.metric === metricName &&
                        a.value === value &&
                        a.status === 'open');
                    if (!existing) {
     ...
     * 使用简单移动平均 + 线性回归进行预测
     */
    generateForecast(tenantId, metric, period) {
        // 从 KPI 中获取历史数据
     ...
     * 获取仪表盘摘要
     */
    getDashboardSummary(tenantId, storeId) {
        const kpiPool = this.kpis.filter((k) => k.tenantId === tenantId && (storeId ? k.storeId === storeId || !k.storeId : true));
        const now = new Date();
        ...
        const buildPeriodSummary = (label, start, end) => {
        ...
        // 活跃异常数
        const activeAnomalies = this.anomalies.filter((a) => a.tenantId === tenantId && a.status !== 'resolved').length;
        return {
            tenantId,
            storeId,
     ...
     * 模拟上期值
     */
    simulatePreviousValue(current, trend) {
     ...
     * 生成摘要文本
     */
    generateSummary(type, metrics, trends, anomalies) {
...
exports.AiInsightService = AiInsightService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [])
], AiInsightService);
//# sourceMappingURL=ai-insight.service.js.map