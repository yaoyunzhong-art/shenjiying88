...
export interface MemberOperationsAction {
    code: 'complete-member-onboarding' | 'send-post-payment-welcome' | 'issue-bounce-back-coupon' | 'recommend-repeat-purchase-bundle' | 'invite-loyalty-challenge' | 'assign-vip-concierge' | 'push-new-arrival-preview' | 'deliver-channel-follow-up';
    label: string;
    reason: string;
...
export interface MemberAutomationTrigger {
    code: 'payment-success-journey' | 'newly-paid-bounce-back' | 'repeat-paid-retention' | 'vip-service-upgrade' | 'channel-retouch';
    status: 'ready' | 'watch';
...
export interface MemberProfileMutationHistoryEntry {
    ...
    tenantContext: RequestTenantContext;
    memberId: string;
    action: 'profile-updated' | 'status-updated' | 'level-updated' | 'points-awarded' | 'points-rolled-back' | 'payment-activity-recorded' | 'approval.approved' | 'approval.rejected' | 'approval.cancelled' | 'approval.executed' | 'approval.execution-failed' | 'approval.resubmitted' | 'approval.superseded';
    summary: string;
 ...
 * 会员等级对应的最低积分阈值
 */
export declare const MEMBER_LEVEL_THRESHOLDS: Record<MemberLevel, number>;
/**
 * 根据积分计算会员等级
 */
export declare function computeMemberLevel(points: number): MemberLevel;
/**
 * 判断会员是否可以升级
 */
export declare function canUpgrade(currentLevel: MemberLevel, points: number): boolean;
/**
 * 构造默认 member bootstrap
 */
export declare function makeMemberBootstrap(tenantContext: RequestTenantContext, overrides?: Partial<Pick<MemberBootstrap, 'capabilities' | 'phase'>>): MemberBootstrap;
