...
exports.MemberService = exports.MEMBER_LEVEL_THRESHOLDS = exports.canUpgrade = exports.computeMemberLevel = exports.MemberStatus = exports.MemberLevel = void 0;
exports.resetMemberServiceTestState = resetMemberServiceTestState;
const node_crypto_1 = require("node:crypto");
const common_1 = require("@nestjs/common");
const prisma_service_1 = require("../../prisma/prisma.service");
const governance_approval_1 = require("../foundation/governance-approval/governance-approval");
const runtime_governance_service_1 = require("../foundation/runtime-governance/runtime-governance.service");
const member_entity_1 = require("./member.entity");
var member_entity_2 = require("./member.entity");
Object.defineProperty(exports, "MemberLevel", { enumerable: true, get: function () { return member_entity_2.MemberLevel; } });
Object.defineProperty(exports, "MemberStatus", { enumerable: true, get: function () { return member_entity_2.MemberStatus; } });
Object.defineProperty(exports, "computeMemberLevel", { enumerable: true, get: function () { return member_entity_2.computeMemberLevel; } });
Object.defineProperty(exports, "canUpgrade", { enumerable: true, get: function () { return member_entity_2.canUpgrade; } });
Object.defineProperty(exports, "MEMBER_LEVEL_THRESHOLDS", { enumerable: true, get: function () { return member_entity_2.MEMBER_LEVEL_THRESHOLDS; } });
/**
 * 内存会员存储（生产环境应替换为 Prisma/数据库）
 */
const memberStore = new Map();
const memberSessionStore = new Map();
const lytMemberSnapshotStore = new Map();
const memberOperationsTaskStore = new Map();
const memberOperationsExecutionStore = new Map();
const memberMutationHistoryStore = new Map();
function resetMemberServiceTestState() {
    memberStore.clear();
...
let MemberService = class MemberService {
    ...
    constructor(prisma, runtimeGovernanceService) {
        this.prisma = prisma;
        this.runtimeGovernanceService = runtimeGovernanceService;
    }
    getLytMemberSnapshotModel() {
    ...
    getMemberOperationsReceiptModel() {
        const prisma = this.prisma;
    ...
    getMemberProfileExtensionModel() {
        const prisma = this.prisma;
    ...
    getLytSnapshotCacheKey(tenantId, externalMemberId) {
    ...
    normalizeOptionalSnapshotBoolean(value) {
        if (typeof value === 'boolean') {
            return value;
        }
        return undefined;
    }
    normalizeSnapshotDateString(value) {
    ...
    parseMemberLevel(value) {
        if (typeof value !== 'string') {
            return undefined;
        }
        const normalized = value.trim().toUpperCase();
        return Object.values(member_entity_1.MemberLevel).find((level) => level === normalized);
    }
    parseMemberStatus(value) {
        if (typeof value !== 'string') {
            return undefined;
        }
        const normalized = value.trim().toUpperCase();
        return Object.values(member_entity_1.MemberStatus).find((status) => status === normalized);
    }
    levelRank(level) {
        return Object.values(member_entity_1.MemberLevel).indexOf(level);
    }
    mapApprovalSnapshot(memberId, approval, summary) {
        return {
            memberId,
            applied: false,
    ...
    async materializeMemberActionApproval(input) {
        if (!this.prisma) {
            return null;
        }
    ...
    getMutationHistoryCacheKey(memberId, tenantId) {
        return `${tenantId}:${memberId}`;
    }
    buildMemberMutationSummary(action, payload) {
        switch (action) {
    ...
    async recordMemberMutationHistory(input) {
        const entry = {
            historyId: `member-history-${(0, node_crypto_1.randomBytes)(6).toString('hex')}`,
        ...
        if (auditLogModel?.findMany) {
            ...
            return records.map((record) => {
                ...
                const action = rawAction.startsWith('member.profile.')
                    ? rawAction.replace('member.profile.', '')
                    : rawAction.startsWith('member.approval.')
                        ? `approval.${rawAction.replace('member.approval.', '')}`
                        : rawAction;
                const metadata = record.metadata ?? {};
                const payload = record.payload ?? {};
                const summary = this.normalizeSnapshotString(metadata.summary) ??
                    (action.startsWith('approval.')
                        ? String(metadata.summary ?? this.buildApprovalOutcomeSummary(payload))
                        : this.buildMemberMutationSummary(action, payload));
                return {
                    ...
                    payload: payload,
                    beforeValue: record.beforeValue && typeof record.beforeValue === 'object'
                        ? record.beforeValue
                        : undefined,
                    afterValue: record.afterValue && typeof record.afterValue === 'object'
                        ? record.afterValue
                        : undefined,
    ...
    buildApprovalOutcomeSummary(payload) {
    ...
    toMemberProfile(input) {
        return {
            ...
            notes: input.notes,
            level: (0, member_entity_1.computeMemberLevel)(input.points),
            status: input.status ?? member_entity_1.MemberStatus.Active,
            points: input.points,
    ...
    toMemberOperationsExecutionReceipt(input) {
        return {
    ...
    toMemberOperationsTaskFromRecord(record) {
        return this.toMemberOperationsTask({
            ...
            actionCode: String(record.actionCode),
            title: String(record.title),
            reason: String(record.reason),
            channel: String(record.channel),
            priority: String(record.priority),
            status: String(record.status),
            executionLane: String(record.executionLane),
    ...
    toMemberOperationsReceiptFromRecord(record) {
        return this.toMemberOperationsExecutionReceipt({
            ...
            runtimeState: this.normalizeSnapshotString(record.runtimeState),
    ...
    buildMemberOperationsTaskPersistenceData(task) {
        return {
            taskId: task.taskId,
    ...
    buildMemberOperationsReceiptPersistenceData(receipt) {
        return {
    ...
    buildOperationsSegments(profile) {
        ...
        return this.uniqueValues([
            ...
            (profile.lastPaymentAmount ?? 0) >= 200 || profile.tags?.includes('high-value-buyer')
                ? 'high-value-buyer'
                : undefined,
            profile.level === member_entity_1.MemberLevel.Gold ||
                profile.level === member_entity_1.MemberLevel.Platinum ||
                profile.level === member_entity_1.MemberLevel.Diamond
                ? 'vip-tier-member'
                : undefined,
    ...
    buildOperationsActions(profile, segments) {
        const actions = [];
        ...
        if (hasSegment('channel-wechat-pay') ||
    ...
    buildAutomationTriggers(profile, segments) {
        const triggers = [];
        const hasSegment = (segment) => segments.includes(segment);
    ...
    buildOperationsProfile(profile) {
    ...
    async findOperationsTaskByDedupeKey(dedupeKey) {
    ...
    async createOperationsTask(task) {
        const taskModel = this.getMemberOperationsTaskModel();
        if (!taskModel?.create) {
    ...
    async updateOperationsTask(task) {
        const taskModel = this.getMemberOperationsTaskModel();
        if (!taskModel?.update) {
            memberOperationsTaskStore.set(task.dedupeKey, task);
            return task;
    ...
    async createOperationsReceipt(receipt) {
    ...
    async updateOperationsReceipt(receipt) {
        const receiptModel = this.getMemberOperationsReceiptModel();
    ...
    async listOperationsTasksForMember(memberId, tenantId) {
        const taskModel = this.getMemberOperationsTaskModel();
    ...
    async listOperationsReceiptsForMember(memberId, tenantId) {
        const receiptModel = this.getMemberOperationsReceiptModel();
    ...
    getExecutionRuntimeAction(task) {
        return task.executionLane === 'promo-conversion' ? 'coupon-claim' : 'booking-submit';
    }
    buildExecutionRuntimePayload(task, receipt) {
        return {
            memberId: task.memberId,
            taskId: task.taskId,
            actionCode: task.actionCode,
    ...
    async attachRuntimeExecutionTrace(task, receipt) {
        ...
        const runtimeReceipt = callbacked.state === 'callback-recorded' ? callbacked : synced;
    ...
    buildCouponOfferPayload(task, profile) {
        const discountAmount = profile.level === member_entity_1.MemberLevel.Diamond || profile.level === member_entity_1.MemberLevel.Platinum ? 60 : 30;
        const expiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString();
        const couponCode = `OPS-${task.memberId.slice(-4).toUpperCase()}-${(0, node_crypto_1.randomBytes)(3)
            .toString('hex')
            .toUpperCase()}`;
        return {
            couponCode,
    ...
    buildCrmFollowUpPayload(task, profile) {
        const ownerQueue = profile.level === member_entity_1.MemberLevel.Diamond || profile.level === member_entity_1.MemberLevel.Platinum
            ? 'vip-concierge'
            : 'member-success';
        return {
            queueId: ownerQueue,
            slaHours: profile.level === member_entity_1.MemberLevel.Diamond ? 2 : 12,
            lifecycleStage: profile.lifecycleStage,
            preferredChannel: profile.lastPaymentChannel ?? task.channel
        };
    }
    async executeOperationsTask(task, profile) {
        const executedAt = new Date().toISOString();
        if (task.executionLane === 'promo-conversion') {
            const payload = this.buildCouponOfferPayload(task, profile);
            const receipt = {
                executionId: `ops-exec-${(0, node_crypto_1.randomBytes)(6).toString('hex')}`,
        ...
        if (task.executionLane === 'member-crm') {
            ...
            const followUpId = `crm-followup-${(0, node_crypto_1.randomBytes)(5).toString('hex')}`;
            const receipt = {
                executionId: `ops-exec-${(0, node_crypto_1.randomBytes)(6).toString('hex')}`,
                tenantContext: task.tenantContext,
                memberId: task.memberId,
    ...
    hydratePersistentProfile(input) {
        const cached = memberStore.get(input.memberProfile.id);
        const nickname = input.snapshot?.nickname ??
            cached?.nickname ??
        ...
        const profile = this.toMemberProfile({
            ...
            source: 'prisma',
            persisted: true,
            status: this.parseMemberStatus(input.snapshot?.status) ?? member_entity_1.MemberStatus.Active
        });
    ...
    async savePersistentSnapshotOverride(input) {
    ...
    async findSnapshotByMemberProfileId(memberProfileId, tenantContext) {
        const snapshotModel = this.getLytMemberSnapshotModel();
        if (!snapshotModel?.findFirst) {
            for (const snapshot of lytMemberSnapshotStore.values()) {
                if (snapshot.memberProfileId === memberProfileId &&
    ...
    async findMemberProfileExtension(memberProfileId) {
    ...
    async saveMemberProfileExtension(input) {
        const extensionModel = this.getMemberProfileExtensionModel();
        const normalized = {
    ...
    getBootstrap(tenantContext) {
        return (0, member_entity_1.makeMemberBootstrap)(tenantContext);
    }
    /**
     * 根据会员 ID 获取会员档案
     */
    ...
    addPoints(memberId, points) {
        ...
        const newLevel = (0, member_entity_1.computeMemberLevel)(profile.points);
        profile.level = newLevel;
        profile.lastActiveAt = new Date().toISOString();
        return profile;
    }
    revokePoints(memberId, points) {
        const profile = memberStore.get(memberId);
        if (!profile) {
            throw new Error(`Member ${memberId} not found`);
        }
        ...
        profile.level = (0, member_entity_1.computeMemberLevel)(profile.points);
        profile.lastActiveAt = new Date().toISOString();
        return profile;
    }
    sanitizeTagSegment(value) {
        return value.trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
    }
    mergeMemberTags(existing, additions) {
    ...
    deriveLifecycleStage(input) {
        if (input.level === member_entity_1.MemberLevel.Gold || input.level === member_entity_1.MemberLevel.Platinum || input.level === member_entity_1.MemberLevel.Diamond) {
            return 'vip-active';
        }
    ...
    async recordPaymentActivity(input) {
        ...
        const nextLevel = (0, member_entity_1.computeMemberLevel)(profile.points);
    ...
    async awardPoints(memberId, points, tenantContext, approvalTicket) {
        if (points <= 0) {
            throw new Error('Points to add must be positive');
        }
        if (!this.prisma) {
    ...
    async rollbackPoints(memberId, points, tenantContext, approvalTicket) {
        if (points <= 0) {
            throw new Error('Points to revoke must be positive');
        }
        if (!this.prisma) {
    ...
    async updatePersistentStatus(memberId, status, tenantContext, approvalTicket) {
        if (!this.prisma) {
        ...
        if (status === member_entity_1.MemberStatus.Blacklisted) {
            const approval = await this.materializeMemberActionApproval({
                memberId,
                tenantContext,
    ...
    async overridePersistentLevel(memberId, level, tenantContext, approvalTicket) {
        if (!this.prisma) {
            const profile = this.getProfile(memberId);
            if (!profile) {
                throw new Error(`Member ${memberId} not found`);
    ...
    checkUpgrade(memberId) {
        ...
        const upgradeable = (0, member_entity_1.canUpgrade)(profile.level, profile.points);
        ...
        const pointsNeeded = nextLevel
            ? member_entity_1.MEMBER_LEVEL_THRESHOLDS[nextLevel] - profile.points
            : 0;
        return {
    ...
    async updatePersistentProfile(input) {
        ...
        const updatedMemberProfile = user?.id !== memberProfile.userId
    ...
    async getOperationsProfile(memberId, tenantContext) {
        const profile = (await this.getPersistentProfile(memberId, tenantContext)) ?? this.getProfile(memberId);
        if (!profile) {
            return undefined;
        }
        if (profile.tenantContext.tenantId !== tenantContext.tenantId) {
    ...
    async enqueueOperationsTasks(input) {
        ...
        for (const action of profile.recommendedActions) {
            ...
            const task = {
                taskId: `ops-task-${(0, node_crypto_1.randomBytes)(6).toString('hex')}`,
                tenantContext: input.tenantContext,
                memberId: input.memberId,
    ...
    async getOperationsRuntimeReceipt(memberId, executionId, tenantContext) {
        const receipt = (await this.listOperationsReceiptsForMember(memberId, tenantContext.tenantId)).find((entry) => entry.executionId === executionId);
    ...
    async replayOperationsExecution(memberId, executionId, tenantContext) {
        ...
        return this.runtimeGovernanceService.replayAction(receipt.runtimeReceiptCode, {
            ...
            idempotencyKey: `member-operations:${executionId}:replay:${(0, node_crypto_1.randomBytes)(4).toString('hex')}`,
            tenantId: tenantContext.tenantId,
    ...
    async listLytMemberSnapshots(tenantContext) {
        ...
        if (!snapshotModel?.findMany) {
            return Array.from(lytMemberSnapshotStore.values()).filter((item) => item.tenantContext.tenantId === tenantContext.tenantId);
        }
        ...
        return records.map((record) => this.toLytMemberSnapshot({
            snapshotId: String(record.id),
            tenantContext: {
                tenantId: String(record.tenantId),
                brandId: this.normalizeSnapshotString(record.brandId),
    ...
    async syncLytMemberSnapshot(input) {
        ...
        const status = this.normalizeSnapshotString(input.status) ?? member_entity_1.MemberStatus.Active;
        ...
        if (!this.prisma) {
            ...
            profile.status = status;
            profile.level = (0, member_entity_1.computeMemberLevel)(points);
        ...
        const snapshotModel = this.getLytMemberSnapshotModel();
        const existingSnapshot = (await this.getLytMemberSnapshot(input.externalMemberId, input.tenantContext)) ?? undefined;
        ...
        const user = normalizedMobile
            ...
            : null;
        let memberProfile = existingSnapshot?.memberProfileId
    ...
    async login(input) {
        ...
        const session = {
            sessionToken: (0, node_crypto_1.randomBytes)(24).toString('hex'),
