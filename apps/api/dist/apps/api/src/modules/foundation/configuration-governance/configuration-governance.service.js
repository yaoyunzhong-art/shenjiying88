...
exports.ConfigurationGovernanceService = void 0;
...
let ConfigurationGovernanceService = class ConfigurationGovernanceService {
    prisma;
    ...
    async listGovernanceApprovals(filters = {}) {
        return (0, governance_approval_1.listGovernanceApprovals)(this.prisma, {
            ...filters,
    ...
    async getGovernanceApprovalDetail(approvalTicket) {
        const approval = await (0, governance_approval_1.getGovernanceApprovalDetail)(this.prisma, approvalTicket);
        if (!this.isConfigurationApproval(approval.operation, approval.resourceType)) {
            throw new common_1.NotFoundException(`Configuration governance approval not found: ${approvalTicket}`);
        }
        return approval;
    }
        ...
        return {
            ...
            configuration: {
                ...
                featureFlags: {
                    ...
                    active: flags.filter((flag) => flag.status === client_1.FeatureFlagStatus.ACTIVE).length,
    ...
    async summarizeGovernanceApprovals(filters = {}) {
        return (0, governance_approval_1.summarizeGovernanceApprovals)(this.prisma, {
            ...filters,
            operationIn: ['config-entry.write', 'feature-flag.write', 'secret.register', 'secret.rotate'],
            resourceTypeIn: ['config-entry', 'feature-flag', 'secret']
        });
    }
    async resolveConfigSnapshot(context) {
        const normalizedContext = this.normalizeContext(context);
        const resolvedConfig = this.deepMerge(this.mergeConfig(normalizedContext), await this.getPersistentConfigFragment(normalizedContext));
    ...
    async getFeatureFlags(context, subjectKey) {
        const normalizedContext = this.normalizeContext(context);
    ...
    async listPersistedFeatureFlags(context, subjectKey) {
        const normalizedContext = this.normalizeContext(context);
        const records = await this.prisma.featureFlag.findMany({
            where: {
                OR: [
                    { scopeType: client_1.FoundationScopeType.PLATFORM },
                    { scopeType: client_1.FoundationScopeType.TENANT, tenantId: normalizedContext.tenantId },
                    { scopeType: client_1.FoundationScopeType.BRAND, brandId: normalizedContext.brandId },
                    { scopeType: client_1.FoundationScopeType.STORE, storeId: normalizedContext.storeId }
                ]
            },
        ...
        return Promise.all([...latestRecords.values()].map((record) => this.evaluateFeatureFlag(record.key, normalizedContext, subjectKey ?? this.getSubjectKey(normalizedContext))));
    }
    async evaluateFeatureFlag(flagKey, context, subjectKey) {
        const normalizedContext = this.normalizeContext(context);
        ...
        if (!flag) {
            throw new common_1.NotFoundException(`Feature flag not found: ${flagKey}`);
        }
    ...
    async getSecretMetadata(secretName) {
        ...
        const persisted = this.groupByKey(persistedRecords).map(([key, records]) => this.toPersistedSecretMetadata(key, records));
        ...
        if (secretName && merged.length === 0) {
            throw new common_1.NotFoundException(`Secret not found: ${secretName}`);
        }
        return merged;
    }
    ...
    async getCertificateDetail(certificateName, filters = {}) {
        const certificate = (await this.getCertificateMetadata({
            name: certificateName,
            expiringWithinDays: filters.expiringWithinDays
        }))[0];
        if (!certificate) {
            throw new common_1.NotFoundException(`Certificate not found: ${certificateName}`);
        }
        return certificate;
    }
    ...
    async listConfigEntries(filters) {
        const entries = await this.prisma.configEntry.findMany({
            where: {
                ...
                OR: [
                    { scopeType: client_1.FoundationScopeType.PLATFORM },
                    filters.tenantId ? { scopeType: client_1.FoundationScopeType.TENANT, tenantId: filters.tenantId } : undefined,
                    filters.brandId ? { scopeType: client_1.FoundationScopeType.BRAND, brandId: filters.brandId } : undefined,
                    filters.storeId ? { scopeType: client_1.FoundationScopeType.STORE, storeId: filters.storeId } : undefined
                ].filter(Boolean)
            },
            include: {
                revisions: {
    ...
    async saveConfigEntry(input) {
        const scopeType = client_1.FoundationScopeType[input.scopeType];
        const valueType = client_1.ConfigValueType[input.valueType];
    ...
    async saveFeatureFlag(input) {
        const scopeType = client_1.FoundationScopeType[input.scopeType];
        const status = client_1.FeatureFlagStatus[input.status];
        const strategy = client_1.RolloutStrategy[input.strategy];
    ...
    async rotateSecret(secretName, rotatedBy = 'configuration-governance-controller', governance) {
        const persistedRecords = await this.prisma.secretAsset.findMany({
        ...
        const rotationRequestPayload = {
            secretName,
            scopeType: (persistedCurrent?.scopeType ?? client_1.FoundationScopeType.PLATFORM),
        ...
        try {
            await this.prisma.secretAsset.create({
                data: {
                    ...
                    provider: persistedCurrent?.provider ?? client_1.SecretProvider.DATABASE,
                    scopeType: persistedCurrent?.scopeType ?? client_1.FoundationScopeType.PLATFORM,
                    tenantId: persistedCurrent?.tenantId ?? null,
                    brandId: persistedCurrent?.brandId ?? null,
                    storeId: persistedCurrent?.storeId ?? null,
                    ...
                    metadata: {
                        ...
                        scopes: this.getStringArray(baseMetadata.scopes).length > 0 ? this.getStringArray(baseMetadata.scopes) : fallbackSecret?.scopes ?? [],
                        consumers: this.getStringArray(baseMetadata.consumers).length > 0
                            ? this.getStringArray(baseMetadata.consumers)
                            : fallbackSecret?.consumers ?? [],
                        algorithm: this.getString(baseMetadata.algorithm) ??
            ...
            finalizedApproval = await (0, governance_approval_1.markGovernanceApprovalExecuted)(this.prisma, {
    ...
    async registerSecret(input) {
        ...
        const provider = input.provider ? client_1.SecretProvider[input.provider] : current?.provider ?? client_1.SecretProvider.DATABASE;
    ...
    mergeConfig(context) {
        ...
        return fragments.reduce((accumulator, fragment) => this.deepMerge(accumulator, fragment), {});
    }
    ...
    matchesScope(scope, context) {
        return Object.entries(scope).every(([key, value]) => context[key] === value);
    }
    getScopeSpecificity(scope) {
        return Object.keys(scope).length;
    }
    computeRolloutBucket(flagKey, subjectKey) {
        const hash = (0, node_crypto_1.createHash)('sha256').update(`${flagKey}:${subjectKey}`).digest('hex').slice(0, 8);
        return Number.parseInt(hash, 16) % 100;
    }
    getSubjectKey(context) {
        return `${context.tenantId}:${context.brandId}:${context.storeId}`;
    }
    buildFingerprint(value) {
        return `sha256:${(0, node_crypto_1.createHash)('sha256').update(value).digest('hex').slice(0, 12)}`;
    }
    buildChecksum(payload) {
        return (0, node_crypto_1.createHash)('sha256').update(JSON.stringify(payload)).digest('hex');
    }
    buildId(prefix, payload) {
        return `${prefix}_${this.buildChecksum(payload).slice(0, 12)}`;
    }
    async getPersistentConfigFragment(context) {
        const entries = await this.prisma.configEntry.findMany({
            where: {
                OR: [
                    { scopeType: client_1.FoundationScopeType.PLATFORM },
                    { scopeType: client_1.FoundationScopeType.TENANT, tenantId: context.tenantId },
                    { scopeType: client_1.FoundationScopeType.BRAND, brandId: context.brandId },
                    { scopeType: client_1.FoundationScopeType.STORE, storeId: context.storeId }
                ]
            }
        });
        ...
        return [...latestEntries.values()]
            ...
            .reduce((accumulator, entry) => this.deepMerge(accumulator, this.toConfigFragment(entry.value, entry.key)), {});
    }
    evaluatePersistedFeatureFlag(flagKey, records, context, subjectKey) {
        ...
        const fallback = records.find((record) => record.scopeType === client_1.FoundationScopeType.PLATFORM) ?? records[0];
    ...
    matchesPersistedFlagScope(record, context) {
        switch (record.scopeType) {
            case client_1.FoundationScopeType.PLATFORM:
                return true;
            case client_1.FoundationScopeType.TENANT:
                return record.tenantId === context.tenantId;
            case client_1.FoundationScopeType.BRAND:
                return record.brandId === context.brandId;
            case client_1.FoundationScopeType.STORE:
                return record.storeId === context.storeId;
            default:
                return false;
        }
    }
    isPersistedFlagEnabled(record, subjectKey, context) {
        const now = Date.now();
        if (record.status !== client_1.FeatureFlagStatus.ACTIVE || !record.enabled) {
            return false;
        }
        ...
        switch (record.strategy) {
            case client_1.RolloutStrategy.ALL:
            case client_1.RolloutStrategy.SCOPE_MATCH:
                return true;
            case client_1.RolloutStrategy.ALLOW_LIST:
                return record.allowList.includes(subjectKey);
            case client_1.RolloutStrategy.PERCENTAGE:
                return this.computeRolloutBucket(`persisted:${context.tenantId}:${subjectKey}`, subjectKey) < (record.percentage ?? 0);
            default:
                return false;
        }
    }
    getPersistedRolloutPercentage(record, subjectKey, context) {
    ...
    getPersistentScopeSpecificity(scopeType) {
        switch (scopeType) {
            case client_1.FoundationScopeType.PLATFORM:
                return 0;
            case client_1.FoundationScopeType.TENANT:
    ...
    async getConfigEntryById(id) {
        ...
        if (!entry) {
            throw new common_1.NotFoundException(`Config entry not found: ${id}`);
        }
        return {
            id: entry.id,
            namespace: entry.namespace,
    ...
    toPersistedSecretMetadata(key, records) {
        if (records.length === 0) {
            throw new common_1.NotFoundException(`Secret not found: ${key}`);
        }
    ...
    toCertificateMetadata(certificate, expiringWithinDays) {
        const now = Date.now();
    ...
    toPrismaSecretKind(secretType) {
        switch (secretType) {
            case 'webhook-signing':
                return client_1.SecretKind.WEBHOOK_SECRET;
            case 'certificate':
                return client_1.SecretKind.CERTIFICATE;
            case 'api-key':
            default:
                return client_1.SecretKind.API_KEY;
        }
    }
    fromPrismaSecretKind(kind) {
        switch (kind) {
            case client_1.SecretKind.WEBHOOK_SECRET:
                return 'webhook-signing';
            case client_1.SecretKind.CERTIFICATE:
                return 'certificate';
            case client_1.SecretKind.API_KEY:
            default:
                return 'api-key';
        }
    }
    getDefaultSecretAlgorithm(kind) {
        switch (kind) {
            case client_1.SecretKind.WEBHOOK_SECRET:
                return 'hmac-sha256';
            case client_1.SecretKind.CERTIFICATE:
    ...
    async recordGovernanceAudit(eventType, input) {
    ...
    isConfigurationApproval(operation, resourceType) {
        return ((operation === 'config-entry.write' ||
            operation === 'feature-flag.write' ||
            operation === 'secret.register' ||
            operation === 'secret.rotate') &&
...
exports.ConfigurationGovernanceService = ConfigurationGovernanceService;
exports.ConfigurationGovernanceService = ConfigurationGovernanceService = __decorate([
...
//# sourceMappingURL=configuration-governance.service.js.map