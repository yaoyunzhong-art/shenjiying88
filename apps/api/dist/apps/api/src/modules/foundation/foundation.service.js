...
let FoundationService = class FoundationService {
    ...
    prisma;
    constructor(identityAccessService, configurationGovernanceService, integrationOrchestrationService, trustGovernanceService, resilienceOperationsService, runtimeGovernanceService, prisma) {
    ...
    async getOperationsOverview(tenantContext) {
        ...
        const approvalsPending = Number(toRecord(toRecord(trustOverview.approvals).statuses).PENDING ?? 0) +
            Number(toRecord(toRecord(configurationOverview.approvals).statuses).PENDING ?? 0);
        const approvalsWithFailures = Number(toRecord(toRecord(trustOverview.approvals).execution).withFailures ?? 0) +
            Number(toRecord(toRecord(configurationOverview.approvals).execution).withFailures ?? 0);
        const highRiskAudits = Number(toRecord(toRecord(trustOverview.audits).byRiskLevel).high ?? 0) +
        ...
        const configurationFailures = toCountEntries(toRecord(toRecord(toRecord(configurationOverview.approvals).execution).byFailureStatus), 'configuration-governance');
    ...
    async getOperationsModuleDetail(moduleKey, tenantContext) {
        const supportedModuleKeys = ['trust-governance', 'configuration-governance', 'resilience-operations', 'runtime-governance'];
        const moduleKeyCandidate = moduleKey;
        ...
        return {
            ...
            health: resolvedModuleKey === 'trust-governance'
    ...
    async acknowledgeOperationsAlert(code, tenantContext, actorContext, note) {
        if (!supportedOperationsAlertCodes.includes(code)) {
            return {
                generatedAt: new Date().toISOString(),
                code,
    ...
    async muteOperationsAlert(code, tenantContext, actorContext, input = {}) {
        if (!supportedOperationsAlertCodes.includes(code)) {
            return {
                generatedAt: new Date().toISOString(),
                code,
    ...
    async unmuteOperationsAlert(code, tenantContext, actorContext, note) {
        if (!supportedOperationsAlertCodes.includes(code)) {
            return {
                generatedAt: new Date().toISOString(),
                code,
    ...
    async getOperationsAlertDrilldown(code, tenantContext) {
        ...
        const overview = await this.getOperationsOverview(tenantContext);
        const alert = buildOperationsAlertFromOverview(code, overview);
        ...
        switch (code) {
            ...
            case 'high-risk-audits': {
                ...
                return {
                    ...
                    detail: {
                        ...
                        limit,
                        audits: [
                            ...trustAudits.map((audit) => ({ module: 'trust-governance', audit })),
                            ...configurationAudits.map((audit) => ({ module: 'configuration-governance', audit }))
                        ]
                    }
                };
            }
            case 'blocked-rate-limit-ledgers': {
                ...
                const now = Date.now();
                const blocked = ledgers.filter((ledger) => {
                    const metadata = typeof ledger.metadata === 'object' && ledger.metadata ? ledger.metadata : {};
                    const blockedUntil = typeof metadata.blockedUntil === 'string'
                        ? Date.parse(String(metadata.blockedUntil))
                        : Number.NaN;
            ...
            case 'secret-rotation-attention': {
                ...
                const now = Date.now();
                const attention = secrets.filter((secret) => {
                ...
                const certificateAttention = certificates.filter((certificate) => certificate.status === 'expiring-soon' || certificate.status === 'expired');
                return {
                    generatedAt: overview.generatedAt,
                    code,
                    catalog,
            ...
            case 'runtime-governance-backlog': {
                const receipts = Array.isArray(toRecord(overview.modules.runtimeGovernance).receipts)
                    ? toRecord(overview.modules.runtimeGovernance).receipts
                    : [];
                return {
                    generatedAt: overview.generatedAt,
                    code,
            ...
            case 'runtime-callback-stalled': {
                const stalledReceipts = Array.isArray(toRecord(overview.modules.runtimeGovernance).stalledReceipts)
                    ? toRecord(overview.modules.runtimeGovernance).stalledReceipts
                    : [];
                return {
                    generatedAt: overview.generatedAt,
                    code,
                    ...
                    detail: {
                        total: alert?.count ?? 0,
                        timeoutThresholds: types_1.runtimeGovernanceCallbackTimeoutThresholds,
                        escalationSummary: {
    ...
    async recordAlertMutationHistory(action, code, tenantId, acknowledgement, visibleInOverview, actorContext) {
    ...
    async getAlertHistory(code, tenantId, limit = 10) {
        ...
        return records.map((record) => toFoundationAlertTimelineEntry(record));
    }
    async getLatestAlertActivityMap(codes, tenantId) {
    ...
    async getAlertAcknowledgementMap(codes, tenantId) {
        ...
        return new Map(acknowledgementRecords.map((record) => [record.code, toFoundationAlertAcknowledgement(record)]));
    }
    async buildOperationsAlertReadModels(alerts, tenantId) {
    ...
    async getAlertAcknowledgement(code, tenantContext) {
        const tenantId = tenantContext?.tenantId ?? 'platform';
...
exports.FoundationService = FoundationService = __decorate([
    ...
    __metadata("design:paramtypes", [identity_access_service_1.IdentityAccessService,
        configuration_governance_service_1.ConfigurationGovernanceService,
        integration_orchestration_service_1.IntegrationOrchestrationService,
        trust_governance_service_1.TrustGovernanceService,
        resilience_operations_service_1.ResilienceOperationsService,
...
function toCountEntries(source, module) {
...
function getOperationsAlertCatalogItem(code) {
...
function getAvailableAlertActions(code, acknowledgement) {
    const catalog = getOperationsAlertCatalogItem(code);
    if (!catalog) {
        return [];
    }
    return [
        ...(catalog.drilldownEnabled ? ['DRILLDOWN'] : []),
...
function isAlertVisibleInOverview(acknowledgement) {
...
function getAlertTriageState(acknowledgement, visibleInOverview) {
    if (acknowledgement?.status === 'MUTED') {
...
function getAlertTriageSummary(triageState, acknowledgement, recentOperation, visibleInOverview) {
...
function buildAlertReadModelState(code, acknowledgement, recentOperation, visibleInOverview) {
...
function buildOperationsAlertFromOverview(code, overview) {
    switch (code) {
        case 'approvals-pending': {
        ...
        case 'secret-rotation-attention': {
            const count = overview.summary.rotationDueSecrets +
        ...
        case 'observability-degradation': {
            const criticalSignals = Number(toRecord(toRecord(overview.modules.resilienceOperations).observability).byStatus
        ...
        case 'runtime-governance-backlog': {
            return overview.summary.runtimeGovernanceBacklog > 0
                ? {
                    severity: overview.summary.highRiskRuntimeBacklog > 0 || overview.summary.runtimeGovernanceBacklog >= 5
                        ? 'high'
                        : 'medium',
                    code,
...
function buildModuleHealth(module, input) {
    const score = Math.max(0, 100 - input.highRiskAudits * 10 - input.pendingApprovals * 5 - input.executionFailures * 15 - input.blockedCount * 8);
    return {
        module,
        score,
