...
exports.AiRecommendService = void 0;
const common_1 = require("@nestjs/common");
let AiRecommendService = class AiRecommendService {
    // ---- 内存存储 ----
    recommendations = [];
    profiles = new Map();
    itemScores = [];
     ...
     */
    getPopularRecommendations(storeId, type, limit = 10) {
        const now = new Date().toISOString();
     ...
     * 冷启动处理：无画像时回退到热门推荐
     */
    getPersonalizedRecommendations(memberId, type, limit = 10) {
     ...
     * 协同过滤：找到相似用户也喜欢的物品
     */
    getSimilarUserRecommendations(memberId, limit) {
        const myScores = this.itemScores.filter((s) => s.memberId === memberId);
        if (myScores.length === 0)
            return [];
        const myItems = new Set(myScores.map((s) => s.itemId));
        // 找到与当前用户评分过相同物品的其他用户
        const otherUserScores = this.itemScores.filter((s) => s.memberId !== memberId && myItems.has(s.itemId));
        ...
        for (const otherId of otherMemberIds) {
            const otherScores = this.itemScores.filter((s) => s.memberId === otherId && !myItems.has(s.itemId) && s.rating >= 4);
    ...
    generateRecommendations(input) {
        ...
        switch (strategy.name) {
            ...
            case 'hybrid':
                if (input.memberId) {
                    // 混合：热门 + 个性化 各取一半
                    const popular = this.getPopularRecommendations(input.storeId, targetType, Math.ceil(maxResults / 2));
                    const personalized = this.getPersonalizedRecommendations(input.memberId, targetType, Math.ceil(maxResults / 2));
    ...
    updateStrategy(id, dto) {
        ...
        if (dto.weights !== undefined)
            existing.config.weights = dto.weights;
        if (dto.fallbackStrategy !== undefined)
            existing.config.fallbackStrategy = dto.fallbackStrategy;
        if (dto.minScore !== undefined)
            existing.config.minScore = dto.minScore;
        if (dto.maxResults !== undefined)
            existing.config.maxResults = dto.maxResults;
     ...
     * 记录推荐转化
     */
    recordConversion(recommendationId) {
     ...
     * 从交互自动更新用户画像
     */
    updateProfileFromInteraction(memberId, interaction) {
    ...
    updateProfile(memberId, dto) {
        ...
        if (dto.preferences) {
            if (dto.preferences.gameTypes)
                profile.preferences.gameTypes = dto.preferences.gameTypes;
            if (dto.preferences.priceRange)
                profile.preferences.priceRange = dto.preferences.priceRange;
            if (dto.preferences.visitFrequency)
                profile.preferences.visitFrequency = dto.preferences.visitFrequency;
            if (dto.preferences.avgSpend !== undefined)
                profile.preferences.avgSpend = dto.preferences.avgSpend;
            if (dto.preferences.favoriteTimeSlot)
                profile.preferences.favoriteTimeSlot = dto.preferences.favoriteTimeSlot;
        }
        if (dto.behaviorTags)
            profile.behaviorTags = dto.behaviorTags;
        profile.lastUpdated = now;
        this.profiles.set(memberId, profile);
        return profile;
    }
...
exports.AiRecommendService = AiRecommendService;
exports.AiRecommendService = AiRecommendService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [])
], AiRecommendService);
//# sourceMappingURL=ai-recommend.service.js.map