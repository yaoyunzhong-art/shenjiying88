...
test('👔 [店长-反例] 店长在 API 不可用时不能处理异常退款，但可查看本地快照', async () => {
  ...
  assert.equal(outcome.state, 'blocked', 'API 不可用时支付提交被阻断');
...
test('🛒 [前台-边界] 前台处理退款金额为 0 的边界情况', async () => {
  ...
  // fallback 下所有 submit 状态为 blocked
  const outcome = submitNativeAppActionPlan(plan);
  assert.equal(outcome.state, 'blocked', 'fallback 下为 blocked 态');
...
test('📢 [营销-反例] 营销人员在 API 离线时无法推送优惠券或查看积分详情', async () => {
  ...
  assert.equal(outcome.state, 'blocked', '离线下为 blocked 态');
...
test('🔄 [交叉场景-边界] 导玩员和前台在同一天处理 5 笔交易后 history 被正确截断', () => {
  ...
  // appendNativeAppSubmitHistory 将新 entry 放在队首后 slice(0, 5)
  // 因此 oldEntries 中靠后的条目（如 OLD-HISTORY-4）会被淘汰
  const receiptCodes = updatedHistory.map((h) => h.receiptCode);
  assert.ok(receiptCodes.includes('OLD-HISTORY-0'), '最旧的记录应保留（在旧数组最前）');
  assert.ok(!receiptCodes.includes('OLD-HISTORY-4'), '最新的旧记录被截断（在旧数组最后）');
});
