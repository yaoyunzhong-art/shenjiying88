...
const _requiredpackages = require("./typescript/required-packages");
    ...
    try {
        ...
        if (((_deps_missing = deps.missing) == null ? void 0 : _deps_missing.length) > 0) {
            const missingPackages = deps.missing.map((pkg)=>({
                    ...pkg,
                    pkg: (0, _requiredpackages.getTypeScriptPackageSpec)(pkg.pkg)
                }));
            if (_ciinfo.isCI) {
                // we don't attempt auto install in CI to avoid side-effects
                // and instead log the error for installing needed packages
                (0, _missingDependencyError.missingDepsError)(dir, missingPackages);
            }
            ...
            await (0, _installdependencies.installDependencies)(dir, missingPackages, true).catch((err)=>{
